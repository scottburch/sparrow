...
*        - $j().w2render     - common render
*        - $j().w2destroy    - common destroy
*        - $j().w2marker     - marker plugin
*        - $j().w2tag        - tag plugin
*        - $j().w2overlay    - overlay plugin
*        - $j().w2menu       - menu plugin
...
var w2utils = (function () {
    ...
    function stripTags (html) {
        ...
        switch (typeof html) {
            case 'number':
                break;
            case 'string':
                html = $j.trim(String(html).replace(/(<([^>]+)>)/ig, ""));
                break;
            case 'object':
    ...
    function transition (div_old, div_new, type, callBack) {
        var width  = $j(div_old).width();
        var height = $j(div_old).height();
        var time   = 0.5;
        ...
        setTimeout(function () {
            if (type === 'slide-down') {
                $j(div_old).css('z-index', '1019');
                $j(div_new).css('z-index', '1020');
            }
            if (div_new) {
                $j(div_new).css({
                    'opacity': '1',
            ...
            if (div_old) {
                ...
                if (div_old.parentNode) $j(div_old.parentNode).css({
                    '-webkit-perspective': '',
                    '-moz-perspective': '',
                    '-ms-perspective': '',
                    '-o-perspective': ''
        ...
        function cross(property, value, none_webkit_value) {
            var isWebkit=!!window.webkitURL; // jQuery no longer supports $j.browser - RR
    ...
    function lock (box, msg, spinner) {
        ...
        var $lock = $j(box).find('.w2ui-lock');
        var mess = $j(box).find('.w2ui-lock-msg');
    ...
    function unlock (box) {
        $j(box).find('.w2ui-lock').remove();
        $j(box).find('.w2ui-lock-msg').remove();
    }
    ...
    function locale (locale) {
        ...
        $j.ajax({
            ...
            success  : function (data, status, xhr) {
                w2utils.settings = $j.extend(true, w2utils.settings, data);
    ...
    function scrollBarSize () {
        ...
        var html =
            ...
            '</div>';
        $j('body').append(html);
        tmp.scrollBarSize = 100 - $j('#_scrollbar_width > div').width();
        $j('#_scrollbar_width').remove();
    ...
    function checkName (params, component) { // was w2checkNameParam
        if (!params || typeof params.name === 'undefined') {
            console.log('ERROR: The parameter "name" is required but not supplied in $j().'+ component +'().');
            return false;
        }
    ...
    function checkUniqueId (id, items, itemsDecription, objName) { // was w2checkUniqueId
        if (!$j.isArray(items)) items = [items];
...
w2utils.event = {
    ...
    on: function (eventData, handler) {
        if (!$j.isPlainObject(eventData)) eventData = { type: eventData };
        eventData = $j.extend({ type: null, execute: 'before', target: null, onComplete: null }, eventData);
        ...
        if (!$j.isArray(this.handlers)) this.handlers = [];
    ...
    trigger: function (eventData) {
        var eventData = $j.extend({ type: null, phase: 'before', target: null }, eventData, {
        ...
        for (var h = this.handlers.length-1; h >= 0; h--) {
            ...
            {
                eventData = $j.extend({}, item.event, eventData);
...
w2utils.keyboard = (function (obj) {
    ...
    function init() {
        $j(document).on('keydown', keydown);
        $j(document).on('mousedown', mousedown);
    }

    function keydown (event) {
        var tag = event.target.tagName;
        if ($j.inArray(tag, ['INPUT', 'SELECT', 'TEXTAREA']) !== -1) return;
        if ($j(event.target).prop('contenteditable') === 'true') return;
        if (!w2ui_name) return;
    ...
    function mousedown (event) {
        var tag = event.target.tagName;
        var obj = $j(event.target).parents('.w2ui-reset');
        if (obj.length > 0) {
...
(function () {
    ...
    $j.fn.w2render = function (name) {
        if ($j(this).length > 0) {
            if (typeof name === 'string' && w2ui[name]) w2ui[name].render($j(this)[0]);
            if (typeof name === 'object') name.render($j(this)[0]);
        }
    };

    $j.fn.w2destroy = function (name) {
        if (!name && this.length > 0) name = this.attr('name');
        if (typeof name === 'string' && w2ui[name]) w2ui[name].destroy();
        if (typeof name === 'object') name.destroy();
    };

    $j.fn.w2marker = function (str) {
        if (str === '' || str == null) { // remove marker
            return $j(this).each(function (index, el) {
    ...
    $j.fn.w2tag = function (text, options) {
        if (!$j.isPlainObject(options)) options = {};
        if (!$j.isPlainObject(options.css)) options.css = {};
        if (typeof options['class'] === 'undefined') options['class'] = '';
        // remove all tags
        if ($j(this).length === 0) {
            $j('.w2ui-tag').each(function (index, elem) {
                var opt = $j(elem).data('options');
                if (opt == null) opt = {};
                $j($j(elem).data('taged-el')).removeClass( opt['class'] );
                clearInterval($j(elem).data('timer'));
                $j(elem).remove();
            });
            return;
        }
        return $j(this).each(function (index, el) {
            ...
            if (text === '' || text == null) {
                $j('#w2ui-tag-'+tagID).css('opacity', 0);
                setTimeout(function () {
                    // remmove element
                    clearInterval($j('#w2ui-tag-'+tagID).data('timer'));
                    $j('#w2ui-tag-'+tagID).remove();
                }, 300);
            } else {
                // remove elements
                clearInterval($j('#w2ui-tag-'+tagID).data('timer'));
                ...
                // insert
                $j('body').append(
                    '<div id="w2ui-tag-'+ tagOrigID +'" class="w2ui-tag '+ ($j(el).parents('.w2ui-popup').length > 0 ? 'w2ui-tag-popup' : '') +
                    '" style=""></div>');

                var timer = setInterval(function () {
                    // monitor if destroyed
                    if ($j(el).length === 0 || ($j(el).offset().left === 0 && $j(el).offset().top === 0)) {
                        clearInterval($j('#w2ui-tag-'+tagID).data('timer'));
                        tmp_hide();
                        return;
                    }
                    // monitor if moved
                    if ($j('#w2ui-tag-'+tagID).data('position') !== ($j(el).offset().left + el.offsetWidth) + 'x' + $j(el).offset().top) {
                        $j('#w2ui-tag-'+tagID).css({
                            '-webkit-transition' : '.2s',
                            '-moz-transition'    : '.2s',
                            '-ms-transition'     : '.2s',
                            '-o-transition'      : '.2s',
                            left: ($j(el).offset().left + el.offsetWidth) + 'px',
                            top: $j(el).offset().top + 'px'
                        }).data('position', ($j(el).offset().left + el.offsetWidth) + 'x' + $j(el).offset().top);
                    }
                }, 100);
                setTimeout(function () {
                    if (!$j(el).offset()) return;
                    $j('#w2ui-tag-'+tagID).css({
                        opacity: '1',
                        left: ($j(el).offset().left + el.offsetWidth) + 'px',
                        top: $j(el).offset().top + 'px'
                    ...
                    .data('position', ($j(el).offset().left + el.offsetWidth) + 'x' + $j(el).offset().top)
                    .data('timer', timer);
                    $j(el).off('keypress', tmp_hide).on('keypress', tmp_hide).off('change', tmp_hide).on('change', tmp_hide)
                        .css(options.css).addClass(options['class']);
                    if (typeof options.onShow === 'function') options.onShow();
                }, 1);
                var originalCSS = '';
                if ($j(el).length > 0) originalCSS = $j(el)[0].style.cssText;
                // bind event to hide it
                function tmp_hide() {
                    $tag = $j('#w2ui-tag-'+tagID);
                    if ($tag.length <= 0) return;
                    clearInterval($tag.data('timer'));
                    $tag.remove();
                    $j(el).off('keypress', tmp_hide).removeClass(options['class']);
                    if ($j(el).length > 0) $j(el)[0].style.cssText = originalCSS;
    ...
    $j.fn.w2overlay = function (html, options) {
        var obj  = this;
        var name = '';
        var defaults = {
        ...
        if (this.length === 0 || options.html === '' || options.html == null) {
            if ($j('#w2ui-overlay'+ name).length > 0) {
                tmp_hide = $j('#w2ui-overlay'+ name)[0].hide;
                if (typeof tmp_hide === 'function') tmp_hide();
            } else {
            $j('#w2ui-overlay'+ name).remove();
            }
            return $j(this);
        }
        if ($j('#w2ui-overlay'+ name).length > 0) {
            tmp_hide = $j('#w2ui-overlay'+ name)[0].hide;
            $j(document).off('click', tmp_hide);
            if (typeof tmp_hide === 'function') tmp_hide();
        }
        $j('body').append(
            '<div id="w2ui-overlay'+ name +'" style="display: none"'+
            '        class="w2ui-reset w2ui-overlay '+ ($j(this).parents('.w2ui-popup, .w2ui-overlay-popup').length > 0 ? 'w2ui-overlay-popup' : '') +'">'+
            '    <style></style>'+
            '    <div style="'+ options.style +'" class="'+ options['class'] +'"></div>'+
            '</div>'
        );
        // init
        var div1 = $j('#w2ui-overlay'+ name);
        ...
        div1.data('element', obj.length > 0 ? obj[0] : null)
            .data('options', options)
            .data('position', $j(obj).offset().left + 'x' + $j(obj).offset().top)
            .fadeIn('fast').on('mousedown', function (event) {
                $j('#w2ui-overlay'+ name).data('keepOpen', true);
        ...
        setTimeout(function () {
            resize();
            $j(document).off('click', hide).on('click', hide);
            if (typeof options.onShow === 'function') options.onShow();
        }, 10);
        ...
        // monitor position
        function monitor() {
            var tmp = $j('#w2ui-overlay'+ name);
            if (tmp.data('element') !== obj[0]) return; // it if it different overlay
            if (tmp.length === 0) return;
            var pos = $j(obj).offset().left + 'x' + $j(obj).offset().top;
        ...
        function resize () {
            ...
            if (div1.length > 0) {
                ...
                switch (options.align) {
                    case 'both':
                        options.left = 17;
                        if (options.width === 0) options.width = w2utils.getSize($j(obj), 'width');
                        break;
                    case 'left':
                        options.left = 17;
                        break;
                    case 'right':
                        ...
                        options.left = w2utils.getSize($j(obj), 'width') - w + 10;
                        break;
                }
                // adjust position
                var tmp = (w - 17) / 2;
                ...
                // $j(window).height() - has a problem in FF20
                var maxHeight = window.innerHeight + $j(document).scrollTop() - div2.offset().top - 7;
                var maxWidth  = window.innerWidth + $j(document).scrollLeft() - div2.offset().left - 7;
                if ((maxHeight > -50 && maxHeight < 210) || options.openAbove === true) {
                    // show on top
                    maxHeight = div2.offset().top - $j(document).scrollTop() - 7;
                    ...
                    div1.css('top', ($j(obj).offset().top - h - 24 + options.top) + 'px');
                ...
                // check width
                w = div2.width();
                maxWidth = window.innerWidth + $j(document).scrollLeft() - div2.offset().left - 7;
    ...
    $j.fn.w2menu = function (menu, options) {
        /*
        ITEM STRUCTURE
            item : {
                id       : null,
        ...
        */
        ...
        if (menu === 'refresh') {
            ...
            if ($j('#w2ui-overlay'+ name).length > 0) {
                options = $j.extend($j.fn.w2menuOptions, options);
                var scrTop = $j('#w2ui-overlay'+ name +' div.menu').scrollTop();
                $j('#w2ui-overlay'+ name +' div.menu').html(getMenuHTML());
                $j('#w2ui-overlay'+ name +' div.menu').scrollTop(scrTop);
                mresize();
            } else {
                $j(this).w2menu(options);
            }
        } else {
            if (arguments.length === 1) options = menu; else options.items = menu;
            if (typeof options !== 'object') options = {};
            options = $j.extend({}, defaults, options);
            $j.fn.w2menuOptions = options;
            if (options.name) name = '-' + options.name;
            ...
            $j.fn.w2menuHandler = function (event, index) {
                ...
                // setTimeout(function () { $j(document).click(); }, 50);
            };
            var html = '';
            if (options.search) {
                html +=
            ...
            html += '<div class="menu" style="position: absolute; top: '+ (options.search ? 40 : 0) + 'px; bottom: 0px; width: 100%; overflow: auto;">' +
                        getMenuHTML() +
                    '</div>';
            var ret = $j(this).w2overlay(html, options);
            setTimeout(function () {
                $j('#w2ui-overlay'+ name +' #menu-search')
                    .on('keyup', change)
                ...
                if (options.search) {
                    if (['text', 'password'].indexOf($j(obj)[0].type) != -1 || $j(obj)[0].tagName == 'texarea') return;
                    $j('#w2ui-overlay'+ name +' #menu-search').focus();                    
                }
            }, 200);
            mresize();
            return ret;
        ...
        function mresize() {
            setTimeout(function () {
                // show selected
                $j('#w2ui-overlay'+ name +' tr.w2ui-selected').removeClass('w2ui-selected');
                var cur        = $j('#w2ui-overlay'+ name +' tr[index='+ options.index +']');
                var scrTop    = $j('#w2ui-overlay'+ name +' div.menu').scrollTop();
                cur.addClass('w2ui-selected');
                if (options.tmp) options.tmp.contentHeight = $j('#w2ui-overlay'+ name +' table').height() + (options.search ? 50 : 10);
                if (options.tmp) options.tmp.contentWidth  = $j('#w2ui-overlay'+ name +' table').width();
                if ($j('#w2ui-overlay'+ name).length > 0) $j('#w2ui-overlay'+ name)[0].resize();
                // scroll into view
                if (cur.length > 0) {
                    var top    = cur[0].offsetTop - 5; // 5 is margin top
                    var el     = $j('#w2ui-overlay'+ name +' div.menu');
                    var height = el.height();
                    $j('#w2ui-overlay'+ name +' div.menu').scrollTop(scrTop);
                    if (top < scrTop || top + cur.height() > scrTop + height) {
                        $j('#w2ui-overlay'+ name +' div.menu').animate({ 'scrollTop': top - (height - cur.height() * 2) / 2 }, 200, 'linear');
                    }
                }
            }, 1);
        }
        ...
        function change(event) {
            ...
            switch (key) {
                case 13: // enter
                    $j('#w2ui-overlay'+ name).remove();
                    $j.fn.w2menuHandler(event, options.index);
                    break;
                case 9:  // tab
                case 27: // escape
                    $j('#w2ui-overlay'+ name).remove();
                    $j.fn.w2menuHandler(event, -1);
                    break;
                case 38: // up
            ...
            if (!cancel) {
                ...
                for (var i in options.items) {
                    ...
                    if (obj.type === 'enum' && $j.inArray(item.id, ids) !== -1) item.hidden = true;
                ...
                if (mitem.hidden !== true) {
                    ...
                    if (typeof txt !== 'undefined' && txt !== '' && !(/^-+$/.test(txt))) {
                        ...
                        menu_html +=
                            ...
                            '        onmousedown="$j(this).parent().find(\'tr\').removeClass(\'w2ui-selected\'); $j(this).addClass(\'w2ui-selected\');"'+
                            '        onclick="event.stopPropagation(); '+
                            '               if ('+ (mitem.disabled === true ? 'true' : 'false') + ') return;'+
                            '               $j(\'#w2ui-overlay'+ name +'\').remove(); '+
                            '               $j.fn.w2menuHandler(event, \''+ f +'\');">'+
                                imgd +
...
(function () {
    var w2grid = function(options) {
        ...
        $j.extend(true, this, w2obj.grid, options);
    };

    // ====================================================
    // -- Registers as a jQuery plugin

    $j.fn.w2grid = function(method) {
        if (typeof method === 'object' || !method ) {
            ...
            $j.extend(object, { postData: {}, records: [], columns: [], searches: [], toolbar: {}, sortData: [], searchData: [], handlers: [] });
            if (object.onExpand != null) object.show.expandColumn = true;
            $j.extend(true, object.toolbar, toolbar);
            // reassign variables
            for (var p in columns)      object.columns[p]       = $j.extend(true, {}, columns[p]);
            for (var p in columnGroups) object.columnGroups[p]  = $j.extend(true, {}, columnGroups[p]);
            for (var p in searches)     object.searches[p]      = $j.extend(true, {}, searches[p]);
            for (var p in searchData)   object.searchData[p]    = $j.extend(true, {}, searchData[p]);
            for (var p in sortData)     object.sortData[p]      = $j.extend(true, {}, sortData[p]);
            object.postData = $j.extend(true, {}, postData);
            ...
            for (var r in records) {
                ...
                object.records[r] = $j.extend(true, {}, records[r]);
            }
            // add searches
            ...
            // render if necessary
            if ($j(this).length !== 0) {
                object.render($j(this)[0]);
            }
            // register new object
            w2ui[object.name] = object;
            return object;

        } else if (w2ui[$j(this).attr('name')]) {
            var obj = w2ui[$j(this).attr('name')];
    ...
    w2grid.prototype = {
        ...
        buttons: {
            ...
            'search'   : { type: 'html',   id: 'w2ui-search',
                            html: '<div class="w2ui-icon icon-search-down w2ui-search-down" title="'+ 'Select Search Field' +'" '+
                                  'onclick="var obj = w2ui[$j(this).parents(\'div.w2ui-grid\').attr(\'name\')]; obj.searchShowFields();"></div>'
                          },
        ...
        add: function (record) {
            if (!$j.isArray(record)) record = [record];
            var added = 0;
            for (var o in record) {
        ...
        set: function (recid, record, noRefresh) { // does not delete existing, but overrides on top of it
            ...
            if (recid == null) {
                for (var r in this.records) {
                    $j.extend(true, this.records[r], record); // recid is the whole record
                }
                ...
                if (isSummary) {
                    $j.extend(true, this.summary[ind], record);
                } else {
                    $j.extend(true, this.records[ind], record);
                }
        ...
        addColumn: function (before, columns) {
            ...
            if (!$j.isArray(columns)) columns = [columns];
        ...
        addSearch: function (before, search) {
            ...
            if (!$j.isArray(search)) search = [search];
            for (var o in search) {
        ...
        localSort: function (silent) {
            ...
            if ($j.isEmptyObject(this.sortData)) return;
            ...
            this.records.sort(function (a, b) {
                ...
                for (var s in obj.sortData) {
                    ...
                    if (typeof aa == 'string') aa = $j.trim(aa.toLowerCase());
                    if (typeof bb == 'string') bb = $j.trim(bb.toLowerCase());
        ...
        addRange: function (ranges) {
            ...
            if (!$j.isArray(ranges)) ranges = [ranges];
            // if it is selection
            for (var r in ranges) {
                ...
                if (ranges[r].name == 'selection') {
                    ...
                    if (sel.length == 0) {
                        ...
                        var td1   = $j('#grid_'+ this.name +'_rec_'+ first.recid + ' td[col='+ first.column +']');
                        var td2   = $j('#grid_'+ this.name +'_rec_'+ last.recid + ' td[col='+ last.column +']');
                    }
                } else { // other range
        ...
        removeRange: function () {
            ...
            for (var a = 0; a < arguments.length; a++) {
                ...
                $j('#grid_'+ this.name +'_'+ name).remove();
        ...
        refreshRanges: function () {
            var obj  = this;
            var time = (new Date()).getTime();
            var rec  = $j('#grid_'+ this.name +'_records');
            for (var r in this.ranges) {
                ...
                var td1   = $j('#grid_'+ this.name +'_rec_'+ first.recid + ' td[col='+ first.column +']');
                var td2   = $j('#grid_'+ this.name +'_rec_'+ last.recid + ' td[col='+ last.column +']');
                if ($j('#grid_'+ this.name +'_'+ rg.name).length == 0) {
                    ...
                    $j('#grid_'+ this.name +'_'+ rg.name).attr('style', rg.style);
                }
                if (td1.length > 0 && td2.length > 0) {
                    $j('#grid_'+ this.name +'_'+ rg.name).css({
            ...
            // add resizer events
            $j(this.box).find('#grid_'+ this.name +'_resizer').off('mousedown').on('mousedown', mouseStart);
            //$j(this.box).find('#grid_'+ this.name +'_resizer').off('selectstart').on('selectstart', function () { return false; }); // fixes chrome cursror bug
            ...
            function mouseStart (event) {
                ...
                $j(document).off('mousemove', mouseMove).on('mousemove', mouseMove);
                $j(document).off('mouseup', mouseStop).on('mouseup', mouseStop);
            }
            ...
            function mouseMove (event) {
                ...
                if (tmp.tagName != 'TD') tmp = $j(tmp).parents('td')[0];
                if (typeof $j(tmp).attr('col') != 'undefined') column = parseInt($j(tmp).attr('col'));
                tmp = $j(tmp).parents('tr')[0];
                recid = $j(tmp).attr('recid');
                // new range
                if (mv.newRange[1].recid == recid && mv.newRange[1].column == column) return;
                var prevNewRange = $j.extend({}, mv.newRange);
                mv.newRange = [{ recid: mv.recid, column: mv.column }, { recid: recid, column: column }];
                // event before
                eventData = obj.trigger($j.extend(eventData, { originalRange: mv.originalRange, newRange : mv.newRange }));
            ...
            function mouseStop (event) {
                ...
                $j(document).off('mousemove', mouseMove);
                $j(document).off('mouseup', mouseStop);
                // event after
                obj.trigger($j.extend(eventData, { phase: 'after' }));
            }
        ...
        select: function () {
            ...
            for (var a = 0; a < arguments.length; a++) {
                ...
                var recEl  = $j('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(recid));
                if (this.selectType == 'row') {
                    ...
                    if ($j.isArray(s) && s.indexOf(col) != -1) continue;
                    // event before
                ...
                // event after
                this.trigger($j.extend(eventData, { phase: 'after' }));
            }
            // all selected?
            if (sel.indexes.length == this.records.length || (this.searchData.length !== 0 && sel.indexes.length == this.last.searchIds.length)) {
                $j('#grid_'+ this.name +'_check_all').prop('checked', true);
            } else {
                $j('#grid_'+ this.name +'_check_all').prop('checked', false);
            }
            this.status();
        ...
        unselect: function () {
            ...
            for (var a = 0; a < arguments.length; a++) {
                ...
                if (this.selectType == 'row') {
                    ...
                    $j('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(recid) + ' > td[col='+ col +']').removeClass('w2ui-selected');
                    unselected++;
                    if (s.length == 0) {
        ...
        search: function (field, value) {
            ...
            if (arguments.length == 0) {
                ...
                for (var s in this.searches) {
                    ...
                    var operator = $j('#grid_'+ this.name + '_operator_'+s).val();
                    var field1   = $j('#grid_'+ this.name + '_field_'+s);
                    var field2   = $j('#grid_'+ this.name + '_field2_'+s);
                    ...
                    if ((value1 != '' && value1 != null) || (typeof value2 != 'undefined' && value2 != '')) {
                        ...
                        if (operator == 'between') {
                            $j.extend(tmp, { value: [value1, value2] });
                        } else if (operator == 'in' && typeof value1 == 'string') {
                            $j.extend(tmp, { value: value1.split(',') });
                        } else if (operator == 'not in' && typeof value1 == 'string') {
                            $j.extend(tmp, { value: value1.split(',') });
                        } else {
                            $j.extend(tmp, { value: value1 });
                        }
                        if (svalue) $j.extend(tmp, { svalue: svalue });
            ...
            if (typeof field == 'string') {
                ...
                if (typeof value != 'undefined') {
                    if (field.toLowerCase() == 'all') {
                        ...
                        var el = $j('#grid_'+ this.name +'_search_all');
                        ...
                        if (search.type == 'list') {
                            ...
                            if (tmp && !$j.isEmptyObject(tmp)) value = tmp.id;
                        }
                        if (value != '') {
                            var op  = 'contains';
                            var val = value;
            ...
            if ($j.isArray(field)) {
                ...
                for (var f in field) {
                    ...
                    searchData.push($j.extend(true, {}, search, data));
                }
            }
            // event before
        ...
        searchOpen: function () {
            ...
            var obj = this;
            // show search
            $j('#tb_'+ this.name +'_toolbar_item_w2ui-search-advanced').w2overlay(
                this.getSearchesHTML(),    {
                    ...
                    onShow  : function () {
                        ...
                        obj.initSearches();
                        $j('#w2ui-overlay-searches-'+ this.name +' .w2ui-grid-searches').data('grid-name', obj.name);
                        var sfields = $j('#w2ui-overlay-searches-'+ this.name +' .w2ui-grid-searches *[rel=search]');
        ...
        searchClose: function () {
            ...
            // hide search
            if ($j('#w2ui-overlay-searches-'+ this.name +' .w2ui-grid-searches').length > 0) {
                $j().w2overlay('', { name: 'searches-'+ this.name });
            }
        },
        ...
        initAllField: function (field, value) {
            ...
            if (field == 'all') {
                ...
                el.w2field(st, $j.extend({}, search.options, { suffix: '', autoFormat: false, selected: value }));
                if (['list', 'enum'].indexOf(search.type) != -1) {
                    this.last.search = '';
                    this.last.item   = '';
                    el.val('');
        ...
        searchReset: function (noRefresh) {
            ...
            // -- clear all search field
            this.searchClose();
            $j('#grid_'+ this.name +'_search_all').val('');
            // apply search
        ...
        skip: function (offset) {
            ...
            if (url) {
                ...
                this.last.scrollTop  = 0;
                this.last.scrollLeft = 0;
                $j('#grid_'+ this.name +'_records').prop('scrollTop',  0);
                this.reload();
            } else {
        ...
        request: function (cmd, add_params, url, callBack) {
            ...
            // append other params
            $j.extend(params, this.postData);
            $j.extend(params, add_params);
            // event before
            if (cmd == 'get-records') {
            ...
            if (this.last.xhr_offset == 0) {
                ...
                var more = $j('#grid_'+ this.name +'_rec_more');
            ...
            if (!$j.isEmptyObject(obj.routeData)) {
            ...
            if (w2utils.settings.dataType == 'HTTP') {
                ajaxOptions.data = (typeof ajaxOptions.data == 'object' ? String($j.param(ajaxOptions.data, false)).replace(/%5B/g, '[').replace(/%5D/g, ']') : ajaxOptions.data);
            }
            ...
            this.last.xhr = $j.ajax(ajaxOptions)
                ...
                .fail(function (xhr, status, error) {
                    ...
                    // event after
                    obj.trigger($j.extend(eventData2, { phase: 'after' }));
                });
            if (cmd == 'get-records') {
                // event after
                this.trigger($j.extend(eventData, { phase: 'after' }));
                ...
                if (typeof responseText != 'undefined' && responseText != '') {
                    ...
                    if (typeof responseText == "object") {
                        ...
                        if (typeof obj.parser == 'function') {
                            ...
                            // $j.parseJSON or $j.getJSON did not work because those expect perfect JSON data - where everything is in double quotes
                            //
        ...
        getChanges: function () {
            ...
            for (var r in this.records) {
                ...
                if (typeof rec['changes'] != 'undefined') {
                    changes.push($j.extend(true, { recid: rec.recid }, rec.changes));
                }
            }
            return changes;
        },
        ...
        editField: function (recid, column, value, event) {
            ...
            var tr = $j('#grid_'+ obj.name +'_rec_'+ w2utils.escapeId(recid));
            ...
            // mormalize items
            if (edit.items.length > 0 && !$j.isPlainObject(edit.items[0])) {
            ...
            if (edit.type == 'select') {
                ...
                $j(input).w2field(edit.type, $j.extend(edit, { selected: val }))
                // add blur listener
                setTimeout(function () {
                    var tmp = input;
                    if (edit.type == 'list') {
                        tmp = $j($j(input).data('w2field').helpers.focus).find('input');
                        if (typeof val != 'object' && val != '') tmp.val(val).css({ opacity: 1 }).prev().css({ opacity: 1 });
                    }
                    $j(tmp).on('blur', function (event) {
                        obj.editChange.call(obj, input, index, column, event);
                    });
                }, 10);
                if (value != null) $j(input).val(val != 'object' ? val : '');
            }
            setTimeout(function () {
        ...
        editChange: function (el, index, column, event) {
            ...
            var tr      = $j('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(rec.recid));
            var col     = this.columns[column];
            var new_val = el.value;
            var old_val = this.parseField(rec, col.field);
            var tmp     = $j(el).data('w2field');
            if (tmp) {
                new_val = tmp.clean(new_val);
                if (tmp.type == 'list' && new_val != '') new_val = $j(el).data('selected');
            }
            ...
            while (true) {
                ...
                if ((typeof new_val != 'object' && String(old_val) != String(new_val)) || 
                    ...
                    // change event
                    eventData = this.trigger($j.extend(eventData, { type: 'change', phase: 'before' }));
                    if (eventData.isCancelled !== true) {
                    ...
                    // restore event
                    eventData = this.trigger($j.extend(eventData, { type: 'restore', phase: 'before' }));
                    if (eventData.isCancelled !== true) {
                        if (new_val !== eventData.value_new) {
                            // re-evaluate the type of change to be made
                            continue;
                        ...
                        if ($j.isEmptyObject(rec.changes)) delete rec.changes;
                        // event after
                        this.trigger($j.extend(eventData, { phase: 'after' }));
                    }
                }
            ...
            if (!summary) {
                if (rec.changes && typeof rec.changes[col.field] != 'undefined') {
                    $j(tr).find('[col='+ column +']').addClass('w2ui-changed').html(cell);
                } else {
                    $j(tr).find('[col='+ column +']').removeClass('w2ui-changed').html(cell);
                }
            }
        },
        ...
        click: function (recid, event) {
            ...
            if (event.shiftKey && sel.length > 0 && obj.multiSelect) {
                ...
                for (var i = start; i <= end; i++) {
                    if (this.searchData.length > 0 && !url && $j.inArray(i, this.last.searchIds) == -1) continue;
                    if (this.selectType == 'row') {
                ...
                if (((!event.ctrlKey && !event.shiftKey && !event.metaKey) || !this.multiSelect) && !this.showSelectColumn) {
                    if (this.selectType != 'row' && $j.inArray(column, last.columns[ind]) == -1) flag = false;
        ...
        keydown: function (event) {
            ...
            // default behavior
            var empty   = false;
            var records = $j('#grid_'+ obj.name +'_records');
            var sel     = obj.getSelection();
            ...
            var recEL  = $j('#grid_'+ obj.name +'_rec_'+ (ind !== null ? w2utils.escapeId(obj.records[ind].recid) : 'none'));
            var cancel = false;
            var key    = event.keyCode;
            ...
            switch (key) {
                ...
                case 70: // cmd + F
                    if (!event.metaKey && !event.ctrlKey) break;
                    $j('#grid_'+ obj.name + '_search_all').focus();
                    cancel = true;
                    break;
                ...
                case 37: // left
                    ...
                    var parent = $j('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(obj.records[ind].recid)).parents('tr');
                    if (parent.length > 0 && String(parent.attr('id')).indexOf('expanded_row') != -1) {
                ...
                case 38: // up
                    ...
                    if (prev != null) {
                        ...
                        if (obj.records[prev].expanded) {
                            var subgrid = $j('#grid_'+ obj.name +'_rec_'+ w2utils.escapeId(obj.records[prev].recid) +'_expanded_row').find('.w2ui-grid');
                        ...
                        var parent = $j('#grid_'+ obj.name +'_rec_'+ w2utils.escapeId(obj.records[ind].recid)).parents('tr');
                        if (parent.length > 0 && String(parent.attr('id')).indexOf('expanded_row') != -1) {
                            var recid = parent.prev().attr('recid');
                            var grid  = parent.parents('.w2ui-grid').attr('name');
                            obj.selectNone();
                ...
                case 40: // down
                    ...
                    if (obj.records[ind2].expanded) {
                        var subgrid = $j('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(obj.records[ind2].recid) +'_expanded_row').find('.w2ui-grid');
                        if (subgrid.length > 0 && w2ui[subgrid.attr('name')]) {
                            obj.selectNone();
                            var grid = subgrid.attr('name');
                            var recs = w2ui[grid].records;
                    ...
                    if (next != null) {
                        ...
                        var parent = $j('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(obj.records[ind2].recid)).parents('tr');
                        if (parent.length > 0 && String(parent.attr('id')).indexOf('expanded_row') != -1) {
                ...
                case 91: // cmd key
                    if (empty) break;
                    var text = obj.copy();
                    $j('body').append('<textarea id="_tmp_copy_data" '+
                        '   onpaste="var obj = this; setTimeout(function () { w2ui[\''+ obj.name + '\'].paste(obj.value); }, 1);" '+
                        '   onkeydown="w2ui[\''+ obj.name +'\'].keydown(event)"'+
                        '   style="position: absolute; top: -100px; height: 1px; width: 1px">'+ text +'</textarea>');
                    $j('#_tmp_copy_data').focus().select();
                    // remove _tmp_copy_data textarea
                    $j(document).on('keyup', tmp_key_down);
                    function tmp_key_down() { 
                        $j('#_tmp_copy_data').remove(); 
                        $j(document).off('keyup', tmp_key_down); 
                    }
                    break;
        ...
        scrollIntoView: function (ind) {
            ...
            var records = $j('#grid_'+ this.name +'_records');
        ...
        dblClick: function (recid, event) {
            ...
            if (column == null && event.target) {
                var tmp = event.target;
                if (tmp.tagName != 'TD') tmp = $j(tmp).parents('td')[0];
                column = parseInt($j(tmp).attr('col'));
            }
            // event before
            ...
            this.selectNone();
            var col = this.columns[column];
            if (col && $j.isPlainObject(col.editable)) {
        ...
        contextMenu: function (recid, event) {
            ...
            if (typeof event == 'undefined') event = { offsetX: 0, offsetY: 0, target: $j('#grid_'+ obj.name +'_rec_'+ recid)[0] };
        ...
        expand: function (recid) {
            ...
            if ($j('#grid_'+ this.name +'_rec_'+ id +'_expanded_row').length > 0) return false;
            if (rec.expanded == 'none') return false;
            // insert expand row
            var tmp = 1 + (this.show.selectColumn ? 1 : 0);
            var addClass = ''; // ($j('#grid_'+this.name +'_rec_'+ w2utils.escapeId(recid)).hasClass('w2ui-odd') ? 'w2ui-odd' : 'w2ui-even');
            $j('#grid_'+ this.name +'_rec_'+ id).after(
            ...
            if (eventData.isCancelled === true) {
                $j('#grid_'+ this.name +'_rec_'+ id +'_expanded_row').remove();
                return;
            }
            // default action
            $j('#grid_'+ this.name +'_rec_'+ id).attr('expanded', 'yes').addClass('w2ui-expanded');
            $j('#grid_'+ this.name +'_rec_'+ id +'_expanded_row').show();
            $j('#grid_'+ this.name +'_cell_'+ this.get(recid, true) +'_expand div').html('<div class="w2ui-spinner" style="width: 16px; height: 16px; margin: -2px 2px;"></div>');
            rec.expanded = true;
            // check if height of expanded row > 5 then remove spinner
            setTimeout(ready, 300);
            function ready() {
                var div1 = $j('#grid_'+ obj.name +'_rec_'+ id +'_expanded');
                var div2 = $j('#grid_'+ obj.name +'_rec_'+ id +'_expanded_row .w2ui-expanded1 > div');
                if (div1.height() < 5) return;
                div1.css('opacity', 1);
                div2.show().css('opacity', 1);
                $j('#grid_'+ obj.name +'_cell_'+ obj.get(recid, true) +'_expand div').html('-');
            }
            // event after
            this.trigger($j.extend(eventData, { phase: 'after' }));
            this.resizeRecords();
        ...
        collapse: function (recid) {
            ...
            if (eventData.isCancelled === true) return;
            // default action
            $j('#grid_'+ this.name +'_rec_'+ id).removeAttr('expanded').removeClass('w2ui-expanded');
            $j('#grid_'+ this.name +'_rec_'+ id +'_expanded').css('opacity', 0);
            $j('#grid_'+ this.name +'_cell_'+ this.get(recid, true) +'_expand div').html('+');
            setTimeout(function () {
                $j('#grid_'+ obj.name +'_rec_'+ id +'_expanded').height('0px');
                setTimeout(function () {
                    $j('#grid_'+ obj.name +'_rec_'+ id +'_expanded_row').remove();
                    delete rec.expanded;
                    // event after
                    obj.trigger($j.extend(eventData, { phase: 'after' }));
                    obj.resizeRecords();
        ...
        resize: function () {
            ...
            if (!this.box || $j(this.box).attr('name') != this.name) return;
            // determine new width and height
            $j(this.box).find('> div')
                .css('width', $j(this.box).width())
                .css('height', $j(this.box).height());
            // event before
        ...
        refreshCell: function (recid, field) {
            ...
            var cell      = $j('#grid_'+ this.name + '_rec_'+ recid +' [col='+ col_ind +']');
        ...
        refreshRow: function (recid) {
            var tr = $j('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(recid));
            if (tr.length != 0) {
                var ind  = this.get(recid, true);
                ...
                $j(tr).replaceWith(this.getRecordHTML(ind, line, isSummary));
        ...
        refresh: function () {
            ...
            // -- header
            if (this.show.header) {
                $j('#grid_'+ this.name +'_header').html(this.header +'&nbsp;').show();
            } else {
                $j('#grid_'+ this.name +'_header').hide();
            }
            // -- toolbar
            if (this.show.toolbar) {
                ...
                if (this.toolbar && this.toolbar.get('w2ui-column-on-off') && this.toolbar.get('w2ui-column-on-off').checked) {
                    ...
                    $j('#grid_'+ this.name +'_toolbar').show();
                ...
                $j('#grid_'+ this.name +'_toolbar').hide();
            }
            // -- make sure search is closed
            this.searchClose();
            // search placeholder
            var el = $j('#grid_'+ obj.name +'_search_all');
            ...
            bodyHTML +=  '<div id="grid_'+ this.name +'_records" class="w2ui-grid-records"'+
                        ...
                        '        $j(\'#grid_'+ this.name +'_columns\')[0].scrollLeft = this.scrollLeft;'+
                        '        $j(\'#grid_'+ this.name +'_summary\')[0].scrollLeft = this.scrollLeft;'+
                        '        obj.scroll(event);">'+
                            this.getRecordsHTML() +
                        '</div>'+
            ...
            $j('#grid_'+ this.name +'_body').html(bodyHTML);
            // show summary records
            if (this.summary.length > 0) {
                $j('#grid_'+ this.name +'_summary').html(this.getSummaryHTML()).show();
            } else {
                $j('#grid_'+ this.name +'_summary').hide();
            }
            // -- footer
            if (this.show.footer) {
                $j('#grid_'+ this.name +'_footer').html(this.getFooterHTML()).show();
            } else {
                $j('#grid_'+ this.name +'_footer').hide();
            }
            // show/hide clear search link
             if (this.searchData.length > 0) {
                $j('#grid_'+ this.name +'_searchClear').show();
            } else {
                $j('#grid_'+ this.name +'_searchClear').hide();
            }
            // all selected?
            var sel = this.last.selection;
            if (sel.indexes.length == this.records.length || (this.searchData.length !== 0 && sel.indexes.length == this.last.searchIds.length)) {
            ...
            // mark selection
            setTimeout(function () {
                var str  = $j.trim($j('#grid_'+ obj.name +'_search_all').val());
                if (str != '') $j(obj.box).find('.w2ui-grid-data > div').w2marker(str);
            }, 50);
            // event after
            this.trigger($j.extend(eventData, { phase: 'after' }));
            obj.resize();
        ...
        render: function (box) {
            ...
            if (typeof box != 'undefined' && box != null) {
                if ($j(this.box).find('#grid_'+ this.name +'_body').length > 0) {
                    $j(this.box)
                        .removeAttr('name')
            ...
            $j(this.box)
                ...
                .html('<div>'+
                      ...
                      '</div>');
            if (this.selectType != 'row') $j(this.box).addClass('w2ui-ss');
            if ($j(this.box).length > 0) $j(this.box)[0].style.cssText += this.style;
            // init toolbar
            this.initToolbar();
            if (this.toolbar != null) this.toolbar.render($j('#grid_'+ this.name +'_toolbar')[0]);
            // reinit search_all
            ...
            // init footer
            $j('#grid_'+ this.name +'_footer').html(this.getFooterHTML());            
            // refresh
            ...
            $j(this.box).on('mousedown', mouseStart);
            $j(this.box).on('selectstart', function () { return false; }); // fixes chrome cursor bug

            // event after
            this.trigger($j.extend(eventData, { phase: 'after' }));
            // attach to resize event
            if ($j('.w2ui-layout').length == 0) { // if there is layout, it will send a resize event
                this.tmp_resize = function (event) { w2ui[obj.name].resize(); }
                $j(window).off('resize', this.tmp_resize).on('resize', this.tmp_resize);
            }
            return (new Date()).getTime() - time;
            ...
            function mouseStart (event) {
                ...
                if (obj.last.userSelect == 'text') {
                    ...
                    $j(obj.box).find('.w2ui-grid-body')
                        .css('user-select', 'none')
                        .css('-webkit-user-select', 'none')
                        .css('-moz-user-select', 'none')
                        .css('-ms-user-select', 'none');
                    $j(this.box).on('selectstart', function () { return false; });
                }
                // regular record select
                if ($j(event.target).parents().hasClass('w2ui-head') || $j(event.target).hasClass('w2ui-head')) return;
                if (obj.last.move && obj.last.move.type == 'expand') return;
                // if altKey - alow text selection
                if (event.altKey) {
                    $j(obj.box).off('selectstart');
                    $j(obj.box).find('.w2ui-grid-body')
                    ...
                    obj.last.move = {
                        ...
                        divX   : 0,
                        divY   : 0,
                        recid  : $j(event.target).parents('tr').attr('recid'),
                        column : (event.target.tagName == 'TD' ? $j(event.target).attr('col') : $j(event.target).parents('td').attr('col')),
                        type   : 'select',
                        ghost  : false,
                        start  : true
                    };
                }
                $j(document).on('mousemove', mouseMove);
                $j(document).on('mouseup', mouseStop);
            }
            ...
            function mouseMove (event) {
                ...
                if (obj.reorderRows == true) {
                    if (!mv.ghost) {
                        ...
                        var recs = $j(obj.box).find('.w2ui-grid-records');
                        ...
                        $j('#grid_'+ obj.name + '_ghost').append(tmp).append(mv.ghost);
                    }
                    var recid = $j(event.target).parents('tr').attr('recid');
                    if (recid != mv.from) {
                        var row1 = $j('#grid_'+ obj.name + '_rec_'+ mv.recid);
                        var row2 = $j('#grid_'+ obj.name + '_rec_'+ recid);
                        if (event.screenY - mv.lastY < 0) row1.after(row2); else row2.after(row1);
                        mv.lastY = event.screenY;
                        mv.to      = recid;
                    }
                    var ghost = $j('#grid_'+ obj.name + '_ghost');
                    var recs  = $j(obj.box).find('.w2ui-grid-records');
                    ghost.css({
                ...
                var newSel= [];
                var recid = (event.target.tagName == 'TR' ? $j(event.target).attr('recid') : $j(event.target).parents('tr').attr('recid'));
                ...
                var col2 = parseInt(event.target.tagName == 'TD' ? $j(event.target).attr('col') : $j(event.target).parents('td').attr('col'));
            ...
            function mouseStop (event) {
                ...
                if (mv && mv.type == 'select') {
                    if (obj.reorderRows == true) {
                        ...
                        $j('#grid_'+ obj.name + '_ghost').remove();
                        obj.refresh();
                    }
                }
                delete obj.last.move;
        ...
        destroy: function () {
            ...
            if (eventData.isCancelled === true) return;
            // remove events
            $j(window).off('resize', this.tmp_resize);
            // clean up
        ...
        initColumnOnOff: function () {
            ...
            var col_html =  '<div class="w2ui-col-on-off">'+
                            ...
                            '</td>'+
                            '<td onclick="w2ui[\''+ obj.name +'\'].columnOnOff(this, event, \'line-numbers\'); $j(\'#w2ui-overlay\')[0].hide();">'+
            ...
            if (url && obj.show.skipRecords) {
                col_html +=
                        ...
                        '               $j(\'#w2ui-overlay\')[0].hide(); '+
                        '            }"> '+ w2utils.lang('Records')+
                        '    </div>'+
                        '</td></tr>';
            }
            col_html += '<tr><td colspan="2" onclick="w2ui[\''+ obj.name +'\'].stateSave(); $j(\'#w2ui-overlay\')[0].hide();">'+
                        '    <div style="cursor: pointer; padding: 4px 8px; cursor: default">'+ w2utils.lang('Save Grid State') + '</div>'+
                        '</td></tr>'+
                        '<tr><td colspan="2" onclick="w2ui[\''+ obj.name +'\'].stateReset(); $j(\'#w2ui-overlay\')[0].hide();">'+
        ...
        initColumnDrag: function ( box ) {
            ...
            $j(obj.box).on('mousedown', dragColStart);
            $j(obj.box).on('mouseup', catchMouseup);
            ...
            function dragColStart ( event ) {
                ...
                _dragData.timeout = setTimeout(function(){
                    ...
                    if ( !$j( event.originalEvent.target ).parents().hasClass( 'w2ui-head' ) ) return;

                    // do nothing if it is an invalid column
                    for ( var i = 0, l = invalidColumns.length; i < l; i++ ){
                        if ( $j( event.originalEvent.target ).parents().hasClass( invalidColumns[ i ] ) ) return;
                    }

                    _dragData.numberPreColumnsPresent = $j( obj.box ).find( preColHeadersSelector ).length;

                    //start event for drag start
                    _dragData.columnHead = origColumn = $j( event.originalEvent.target ).parents( '.w2ui-head' );
                    ...
                    columns = _dragData.columns = $j( obj.box ).find( '.w2ui-head:not(.w2ui-head-last)' );

                    //add events
                    $j( document ).on( 'mouseup', dragColEnd );
                    $j( document ).on( 'mousemove', dragColOver );

                    _dragData.originalPos = parseInt( $j( event.originalEvent.target ).parent( '.w2ui-head' ).attr( 'col' ), 10 );
                    //_dragData.columns.css({ overflow: 'visible' }).children( 'div' ).css({ overflow: 'visible' });

                    //configure and style ghost image
                    _dragData.ghost = $j( self ).clone( true );

                    //hide other elements on ghost except the grid body
                    $j( _dragData.ghost ).find( '[col]:not([col="' + _dragData.originalPos + '"]), .w2ui-toolbar, .w2ui-grid-header' ).remove();
                    $j( _dragData.ghost ).find( preColumnsSelector ).remove();
                    $j( _dragData.ghost ).find( '.w2ui-grid-body' ).css({ top: 0 });

                    selectedCol = $j( _dragData.ghost ).find( '[col="' + _dragData.originalPos + '"]' );
                    $j( document.body ).append( _dragData.ghost );

                    $j( _dragData.ghost ).css({
                        width: 0,
                        height: 0,
                        margin: 0,
                        position: 'fixed',
                        ...
                        opacity: 0
                            ...
                            height: $j(obj.box).find('.w2ui-grid-body:first').height(),
                            left : event.pageX,
                            top : event.pageY,
                            opacity: .8
                        }, 0 );
                    ...
                    for ( var i = 0, l = columns.length; i < l; i++ ) {
                        _dragData.offsets.push( $j( columns[ i ] ).offset().left );
                    }
            ...
            function dragColOver ( event ) {
                ...
                var cursorX = event.originalEvent.pageX,
                    ...
                    lastWidth = $j( '.w2ui-head:not(.w2ui-head-last)' ).width();
            ...
            function dragColEnd ( event ) {
                ...
                targetColumn =  $j( _dragData.columns[ Math.min(_dragData.lastInt, _dragData.columns.length - 1) ] );
                ...
                if ( target !== _dragData.originalPos + 1 && target !== _dragData.originalPos && targetColumn && targetColumn.length ) {
                    $j( _dragData.ghost ).animate({
                        top: $j( obj.box ).offset().top,
                    ...
                    columnConfig.splice( target, 0, $j.extend( {}, selected ) );
                    columnConfig.splice( columnConfig.indexOf( selected ), 1);

                } else {
                    $j( _dragData.ghost ).remove();
                    ghosts.remove();
                }

                //_dragData.columns.css({ overflow: '' }).children( 'div' ).css({ overflow: '' });

                $j( document ).off( 'mouseup', dragColEnd );
                $j( document ).off( 'mousemove', dragColOver );
                ...
                //conclude event
                obj.trigger( $j.extend( eventData, { phase: 'after', targetColumnNumber: target - 1 } ) );
            }

            function markIntersection( intersection ){
                if ( !_dragData.marker && !_dragData.markerLeft ) {
                    _dragData.marker = $j('<div class="col-intersection-marker">' +
                        '<div class="top-marker"></div>' +
                        '<div class="bottom-marker"></div>' +
                        '</div>');
                    _dragData.markerLeft = $j('<div class="col-intersection-marker">' +
                        '<div class="top-marker"></div>' +
                        '<div class="bottom-marker"></div>' +
                        '</div>');
                }
                ...
                if ( !_dragData.lastInt || _dragData.lastInt !== intersection ){
                    ...
                    $j('.w2ui-head').removeClass('w2ui-col-intersection');
                    ...
                    if ( intersection >= _dragData.columns.length ) {
                        $j( _dragData.columns[ _dragData.columns.length - 1 ] ).children( 'div:last' ).append( _dragData.marker.addClass( 'right' ).removeClass( 'left' ) );
                        $j( _dragData.columns[ _dragData.columns.length - 1 ] ).addClass('w2ui-col-intersection');
                        ...
                        $j( _dragData.columns[ _dragData.numberPreColumnsPresent ] ).prepend( _dragData.marker.addClass( 'left' ).removeClass( 'right' ) ).css({ position: 'relative' });
                        $j( _dragData.columns[ _dragData.numberPreColumnsPresent ] ).prev().addClass('w2ui-col-intersection');
                    } else {
                        //otherwise prepend the marker to the targeted column and append it to the previous column
                        $j( _dragData.columns[intersection] ).children( 'div:last' ).prepend( _dragData.marker.addClass( 'left' ).removeClass( 'right' ) );
                        $j( _dragData.columns[intersection] ).prev().children( 'div:last' ).append( _dragData.markerLeft.addClass( 'right' ).removeClass( 'left' ) ).css({ position: 'relative' });
                        $j( _dragData.columns[intersection - 1] ).addClass('w2ui-col-intersection');
                    }
                }
            }
            ...
            return {
                remove: function(){
                    $j( obj.box ).off( 'mousedown', dragColStart );
                    $j( obj.box ).off( 'mouseup', catchMouseup );
                    $j( obj.box ).find( '.w2ui-head' ).removeAttr( 'draggable' );
        ...
        initToolbar: function () {
            ...
            if (typeof this.toolbar['render'] == 'undefined') {
                ...
                this.toolbar.items = [];
                this.toolbar = $j().w2toolbar($j.extend(true, {}, this.toolbar, { name: this.name +'_toolbar', owner: this }));

                // =============================================
                // ------ Toolbar Generic buttons

                if (this.show.toolbarReload) {
                    this.toolbar.items.push($j.extend(true, {}, this.buttons['reload']));
                }
                if (this.show.toolbarColumns) {
                    this.toolbar.items.push($j.extend(true, {}, this.buttons['columns']));
                }
                ...
                if (this.show.toolbarSearch) {
                    var html =
                        ...
                        '            onchange="'+
                        '                var val = this.value; '+
                        '                var fld = $j(this).data(\'w2field\'); '+
                        '                var dat = $j(this).data(\'selected\'); '+
                        '                if (fld) val = fld.clean(val);'+
                        '                if (dat != null && $j.isPlainObject(dat)) val = dat.id;'+
                    ...
                    if (this.multiSearch && this.searches.length > 0) {
                        this.toolbar.items.push($j.extend(true, {}, this.buttons['search-go']));
                    }
                }
                ...
                if (this.show.toolbarAdd) {
                    this.toolbar.items.push($j.extend(true, {}, this.buttons['add']));
                }
                if (this.show.toolbarEdit) {
                    this.toolbar.items.push($j.extend(true, {}, this.buttons['edit']));
                }
                if (this.show.toolbarDelete) {
                    this.toolbar.items.push($j.extend(true, {}, this.buttons['delete']));
                }
                if (this.show.toolbarSave) {
                    if (this.show.toolbarAdd || this.show.toolbarDelete || this.show.toolbarEdit) {
                        this.toolbar.items.push({ type: 'break', id: 'w2ui-break2' });
                    }
                    this.toolbar.items.push($j.extend(true, {}, this.buttons['save']));
                }
                    ...
                    switch (id) {
                        ...
                        case 'w2ui-search-advanced':
                            ...
                            if (it.checked) {
                                ...
                                function tmp_close() {
                                    if ($j('#w2ui-overlay-searches-'+ obj.name).data('keepOpen') === true) return;
                                    tb.uncheck(id);
                                    $j(document).off('click', 'body', tmp_close);
                                }
                                $j(document).on('click', 'body', tmp_close);
                            }
                            break;
                        case 'w2ui-add':
                            // events
        ...
        initResize: function () {
            ...
            $j(this.box).find('.w2ui-resizer')
                ...
                .on('mousedown', function (event) {
                    ...
                    eventData = obj.trigger($j.extend(eventData, { resizeBy: 0, originalEvent: event }));
                    // set move event
                    var mouseMove = function (event) {
                        if (obj.resizing != true) return;
                        if (!event) event = window.event;
                        // event before
                        eventData = obj.trigger($j.extend(eventData, { resizeBy: (event.screenX - obj.last.tmp.gx), originalEvent: event }));
                    ...
                    var mouseUp = function (event) {
                        delete obj.resizing;
                        $j(document).off('mousemove', 'body');
                        $j(document).off('mouseup', 'body');
                        obj.resizeRecords();
                        // event before
                        obj.trigger($j.extend(eventData, { phase: 'after', originalEvent: event }));
                    }
                    $j(document).on('mousemove', 'body', mouseMove);
                    $j(document).on('mouseup', 'body', mouseUp);
                })
        ...
        resizeBoxes: function () {
            // elements
            var main     = $j(this.box).find('> div');
            var header   = $j('#grid_'+ this.name +'_header');
            var toolbar  = $j('#grid_'+ this.name +'_toolbar');
            var summary  = $j('#grid_'+ this.name +'_summary');
            var footer   = $j('#grid_'+ this.name +'_footer');
            var body     = $j('#grid_'+ this.name +'_body');
            var columns  = $j('#grid_'+ this.name +'_columns');
            var records  = $j('#grid_'+ this.name +'_records');
        ...
        resizeRecords: function () {
            ...
            $j(this.box).find('.w2ui-empty-record').remove();
            // -- Calculate Column size in PX
            var box     = $j(this.box);
            var grid    = $j(this.box).find('> div');
            var header  = $j('#grid_'+ this.name +'_header');
            var toolbar = $j('#grid_'+ this.name +'_toolbar');
            var summary = $j('#grid_'+ this.name +'_summary');
            var footer  = $j('#grid_'+ this.name +'_footer');
            ...
            if (!this.fixedBody) {
                ...
                var calculatedHeight = w2utils.getSize(columns, 'height')
                    + w2utils.getSize($j('#grid_'+ obj.name +'_records table'), 'height');
            ...
            if (body.width() < $j(records).find('>table').width()) bodyOverflowX = true;
            if (body.height() - columns.height() < $j(records).find('>table').height() + (bodyOverflowX ? w2utils.scrollBarSize() : 0)) bodyOverflowY = true;
            ...
            if (this.show.emptyRecords && !bodyOverflowY) {
                ...
                if (this.fixedBody) {
                    for (var di = buffered; di <= max; di++) {
                        ...
                        html += '</tr>';
                        $j('#grid_'+ this.name +'_records > table').append(html);
                    }
                }
            }
            if (body.length > 0) {
            ...
            columns.find('> table > tbody > tr:nth-child(1) td').each(function (index, el) {
                var ind = $j(el).attr('col');
                if (typeof ind != 'undefined' && obj.columns[ind]) $j(el).css('width', obj.columns[ind].sizeCalculated);
                // last column
                if ($j(el).hasClass('w2ui-head-last')) {
                    $j(el).css('width', w2utils.scrollBarSize() + (width_diff > 0 && percent == 0 ? width_diff : 0) + 'px');
                }
            });
            ...
            records.find('> table > tbody > tr:nth-child(1) td').each(function (index, el) {
                ...
                if (typeof ind != 'undefined' && obj.columns[ind]) $j(el).css('width', obj.columns[ind].sizeCalculated);
                // last column
                if ($j(el).hasClass('w2ui-grid-data-last')) {
                    $j(el).css('width', (width_diff > 0 && percent == 0 ? width_diff : 0) + 'px');
                }
            });
            // resize summary
        ...
        initOperator: function (el, search_ind) {
            ...
            var range   = $j('#grid_'+ obj.name + '_range_'+ search_ind);
            var fld1    = $j('#grid_'+ obj.name +'_field_'+ search_ind);
            var fld2    = fld1.parent().find('span input');
            if ($j(el).val() == 'in' || $j(el).val() == 'not in') { fld1.w2field('clear'); } else { fld1.w2field(search.type); }
            if ($j(el).val() == 'between') { range.show(); fld2.w2field(search.type); } else { range.hide(); }
        },
        ...
        initSearches: function () {
            ...
            for (var s in this.searches) {
                ...
                switch (search.type) {
                    case 'text':
                    case 'alphanumeric':
                        $j('#grid_'+ this.name +'_operator_'+s).val('begins');
                        if (['alphanumeric', 'hex'].indexOf(search.type) != -1) {
                            $j('#grid_'+ this.name +'_field_' + s).w2field(search.type, search.options);
                        }
                        break;
                    ...
                    case 'time':
                        ...
                        $j('#grid_'+ this.name +'_field_'+s).w2field(search.type, search.options);
                        $j('#grid_'+ this.name +'_field2_'+s).w2field(search.type, search.options);
                        setTimeout(function () { // convert to date if it is number
                            $j('#grid_'+ obj.name +'_field_'+s).keydown(); 
                            $j('#grid_'+ obj.name +'_field2_'+s).keydown(); 
                        }, 1);
                        break;
                    ...
                    case 'enum':
                        ...
                        $j('#grid_'+ this.name +'_field_'+s).w2field(search.type, options);
                    ...
                    case 'select':
                        ...
                        for (var i in search.options.items) {
                            ...
                            if ($j.isPlainObject(search.options.items[i])) {
                                var val = si.id;
                                var txt = si.text;
                        ...
                        $j('#grid_'+ this.name +'_field_'+s).html(options);
                        break;
                }
                if (sdata != null) {
                    if (sdata.type == 'int' && ['in', 'not in'].indexOf(sdata.operator) != -1) {
                        $j('#grid_'+ this.name +'_field_'+ s).w2field('clear').val(sdata.value);
                    }
                    $j('#grid_'+ this.name +'_operator_'+ s).val(sdata.operator).trigger('change');
                    if (!$j.isArray(sdata.value)) {
                        if (typeof sdata.value != 'udefined') $j('#grid_'+ this.name +'_field_'+ s).val(sdata.value).trigger('change');
                    } else {
                        if (['in', 'not in'].indexOf(sdata.operator) != -1) {
                            $j('#grid_'+ this.name +'_field_'+ s).val(sdata.value).trigger('change');
                        } else {
                            $j('#grid_'+ this.name +'_field_'+ s).val(sdata.value[0]).trigger('change');
                            $j('#grid_'+ this.name +'_field2_'+ s).val(sdata.value[1]).trigger('change');
                        }
                    }
                }
            }
            // add on change event
            $j('#w2ui-overlay-searches-'+ this.name +' .w2ui-grid-searches *[rel=search]').on('keypress', function (evnt) {
                if (evnt.keyCode == 13) {
                    obj.search();
                    $j().w2overlay();
                }
        ...
        scroll: function (event) {
            ...
            var url = (typeof this.url != 'object' ? this.url : this.url.get);
            $j('#grid_'+ this.name + '_footer .w2ui-footer-right').html(w2utils.formatNumber(this.offset + t1) + '-' + w2utils.formatNumber(this.offset + t2) + ' ' + w2utils.lang('of') + ' ' +    w2utils.formatNumber(this.total) +
            ...
            //$j('#log').html('buffer: '+ this.buffered +' start-end: ' + start + '-'+ end + ' ===> first-last: ' + first + '-' + last);
                    ...
                    if (more.css('display') == 'none') {
                        more.show()
                            .on('click', function () {
                                ...
                                $j(this).find('td').html('<div><div style="width: 20px; height: 20px;" class="w2ui-spinner"></div></div>');
                            });
                    }
            ...
            // check for grid end
            if (buffered >= this.total - this.offset) $j('#grid_'+ this.name +'_rec_more').hide();
            return;
            ...
            function markSearch() {
                ...
                obj.last.marker_timer = setTimeout(function () {
                    ...
                    for (var s in obj.searchData) {
                        ...
                        if ($j.inArray(tmp.value, str) == -1) str.push(tmp.value);
                    }
                    if (str.length > 0) $j(obj.box).find('.w2ui-grid-data > div').w2marker(str);
                }, 50);
            }
        },
        ...
        getRecordHTML: function (ind, lineNum, summary) {
            ...
            while (true) {
                ...
                if (isRowSelected && $j.inArray(col_ind, sel.columns[ind]) != -1) isCellSelected = true;
        ...
        getCellHTML: function (ind, col_ind, summary) {
            ...
            if (typeof col.render != 'undefined') {
                if (typeof col.render == 'function') {
                    data = $j.trim(col.render.call(this, record, ind, col_ind));
        ...
        status: function (msg) {
            if (typeof msg != 'undefined') {
                $j('#grid_'+ this.name +'_footer').find('.w2ui-footer-left').html(msg);
            } else {
                // show number of selected
                var msgLeft = '';
                var sel = this.getSelection();
                ...
                $j('#grid_'+ this.name +'_footer .w2ui-footer-left').html(msgLeft);
                // toolbar
                if (sel.length == 1) this.toolbar.enable('w2ui-edit'); else this.toolbar.disable('w2ui-edit');
                if (sel.length >= 1) this.toolbar.enable('w2ui-delete'); else this.toolbar.disable('w2ui-delete');
            }
        ...
        lock: function (msg, showSpinner) {
            var box  = $j(this.box).find('> div:first-child');
        ...
        stateSave: function (returnOnly) {
            ...
            for (var i in this.sortData) state.sortData.push($j.extend({}, this.sortData[i]));
            for (var i in this.searchData) state.searchData.push($j.extend({}, this.searchData[i]));
            ...
            if (returnOnly !== true) {
                ...
                if (eventData.isCancelled === true) { if (typeof callBack == 'function') callBack({ status: 'error', message: 'Request aborted.' }); return; }
                try {
                    var savedState = $j.parseJSON(localStorage.w2ui || '{}');
        ...
        stateRestore: function (newState) {
            ...
            if (!newState) {
                ...
                try {
                    ...
                    var tmp = $j.parseJSON(localStorage.w2ui || '{}');
                    if (!tmp) tmp = {};
            ...
            if (eventData.isCancelled === true) { if (typeof callBack == 'function') callBack({ status: 'error', message: 'Request aborted.' }); return; }
            // default behavior
            if ($j.isPlainObject(newState)) {
                $j.extend(this.show, newState.show);
                $j.extend(this.last, newState.last);
        ...
        nextRow: function (ind) {
            if ((ind + 1 < this.records.length && this.last.searchIds.length == 0) // if there are more records
                ...
                if (this.last.searchIds.length > 0) {
                    while (true) {
                        if ($j.inArray(ind, this.last.searchIds) != -1 || ind > this.records.length) break;
                        ind++;
                    }
                }
                return ind;
        ...
        prevRow: function (ind) {
            if ((ind > 0 && this.last.searchIds.length == 0)  // if there are more records
                ...
                if (this.last.searchIds.length > 0) {
                    while (true) {
                        if ($j.inArray(ind, this.last.searchIds) != -1 || ind < 0) break;
                        ind--;
                    }
                }
                return ind;
    ...
    $j.extend(w2grid.prototype, w2utils.event);
    w2obj.grid = w2grid;
})();
...
(function () {
    var w2layout = function (options) {
        ...
        $j.extend(true, this, w2obj.layout, options);
    };
    ...
    $j.fn.w2layout = function(method) {
        if (typeof method === 'object' || !method ) {
            // check name parameter
            if (!w2utils.checkName(method, 'w2layout')) return;
            var panels = method.panels || [];
            var object = new w2layout(method);
            $j.extend(object, { handlers: [], panels: [] });
            // add defined panels
            for (var p = 0, len = panels.length; p < len; p++) {
                object.panels[p] = $j.extend(true, {}, w2layout.prototype.panel, panels[p]);
                if ($j.isPlainObject(object.panels[p].tabs) || $j.isArray(object.panels[p].tabs)) initTabs(object, panels[p].type);
                if ($j.isPlainObject(object.panels[p].toolbar) || $j.isArray(object.panels[p].toolbar)) initToolbar(object, panels[p].type);
            }
            // add all other panels
            for (var p1 in w2layout_panels) {
                p1 = w2layout_panels[p1];
                if (object.get(p1) !== null) continue;
                object.panels.push($j.extend(true, {}, w2layout.prototype.panel, { type: p1, hidden: (p1 !== 'main'), size: 50 }));
            }
            if ($j(this).length > 0) {
                object.render($j(this)[0]);
            }
        ...
        function initTabs(object, panel, tabs) {
            ...
            // instanciate tabs
            if ($j.isArray(tabs)) tabs = { tabs: tabs };
            $j().w2destroy(object.name + '_' + panel + '_tabs'); // destroy if existed
            pan.tabs = $j().w2tabs($j.extend({}, tabs, { owner: object, name: object.name + '_' + panel + '_tabs' }));
            pan.show.tabs = true;
            return true;
        }
        ...
        function initToolbar(object, panel, toolbar) {
            ...
            // instanciate toolbar
            if ($j.isArray(toolbar)) toolbar = { items: toolbar };
            $j().w2destroy(object.name + '_' + panel + '_toolbar'); // destroy if existed
            pan.toolbar = $j().w2toolbar($j.extend({}, toolbar, { owner: object, name: object.name + '_' + panel + '_toolbar' }));
    ...
    w2layout.prototype = {
        ...
        content: function (panel, data, transition) {
            ...
            // if it is CSS panel
            if (panel == 'css') {
                $j('#layout_'+ obj.name +'_panel_css').html('<style>'+ data +'</style>');
                return true;
            }
            ...
            if (typeof data == 'undefined' || data === null) {
                ...
                var current = $j(pname + '> .w2ui-panel-content');
                var panelTop = 0;
                if (current.length > 0) {
                    $j(pname).scrollTop(0);
                    panelTop = $j(current).position().top;
                }
                if (p.content === '') {
                    p.content = data;
                    this.refresh(panel);
                    ...
                    if (!p.hidden) {
                        if (transition !== null && transition !== '' && typeof transition != 'undefined') {
                            // apply transition
                            var div1 = $j(pname + '> .w2ui-panel-content');
                            div1.after('<div class="w2ui-panel-content new-panel" style="'+ div1[0].style.cssText +'"></div>');
                            var div2 = $j(pname + '> .w2ui-panel-content.new-panel');
        ...
        load: function (panel, url, transition, onLoad) {
            var obj = this;
            if (panel == 'css') {
                $j.get(url, function (data, status, xhr) { // should always be $j.get as it is template
        ...
        sizeTo: function (panel, size) {
            ...
            // resize
            $j(obj.box).find(' > div > .w2ui-panel').css({
                '-webkit-transition' : '.2s',
                '-moz-transition'    : '.2s',
                '-ms-transition'     : '.2s',
                '-o-transition'      : '.2s'
        ...
        show: function (panel, immediate) {
            ...
            p.hidden = false;
            if (immediate === true) {
                $j('#layout_'+ obj.name +'_panel_'+panel).css({ 'opacity': '1' });
                if (p.resizable) $j('#layout_'+ obj.name +'_resizer_'+panel).show();
                obj.trigger($j.extend(eventData, { phase: 'after' }));
                obj.resize();
            } else {
                if (p.resizable) $j('#layout_'+ obj.name +'_resizer_'+panel).show();
                // resize
                $j('#layout_'+ obj.name +'_panel_'+panel).css({ 'opacity': '0' });
                $j(obj.box).find(' > div > .w2ui-panel').css({
                    '-webkit-transition' : '.2s',
                    '-moz-transition'    : '.2s',
                    '-ms-transition'     : '.2s',
        ...
        hide: function (panel, immediate) {
            ...
            if (immediate === true) {
                $j('#layout_'+ obj.name +'_panel_'+panel).css({ 'opacity': '0'    });
                $j('#layout_'+ obj.name +'_resizer_'+panel).hide();
                obj.trigger($j.extend(eventData, { phase: 'after' }));
                obj.resize();
            } else {
                $j('#layout_'+ obj.name +'_resizer_'+panel).hide();
        ...
        set: function (panel, options) {
            ...
            $j.extend(this.panels[obj], options);
        ...
        el: function (panel) {
            var el = $j('#layout_'+ this.name +'_panel_'+ panel +'> .w2ui-panel-content');
        ...
        hideToolbar: function (panel) {
            ...
            $j('#layout_'+ this.name +'_panel_'+ panel +'> .w2ui-panel-toolbar').hide();
            this.resize();
        },
        ...
        showToolbar: function (panel) {
            ...
            if (!pan) return;
            pan.show.toolbar = true;
            $j('#layout_'+ this.name +'_panel_'+ panel +'> .w2ui-panel-toolbar').show();
            this.resize();
        },
        ...
        hideTabs: function (panel) {
            ...
            if (!pan) return;
            pan.show.tabs = false;
            $j('#layout_'+ this.name +'_panel_'+ panel +'> .w2ui-panel-tabs').hide();
            this.resize();
        },
        ...
        showTabs: function (panel) {
            ...
            if (!pan) return;
            pan.show.tabs = true;
            $j('#layout_'+ this.name +'_panel_'+ panel +'> .w2ui-panel-tabs').show();
            this.resize();
        },
        ...
        render: function (box) {
            ...
            if (typeof box != 'undefined' && box !== null) {
                if ($j(obj.box).find('#layout_'+ obj.name +'_panel_main').length > 0) {
                    $j(obj.box)
                        .removeAttr('name')
            ...
            $j(obj.box)
                ...
                .html('<div></div>');
            if ($j(obj.box).length > 0) $j(obj.box)[0].style.cssText += obj.style;
            // create all panels
            for (var p1 in w2layout_panels) {
                p1 = w2layout_panels[p1];
                var pan  = obj.get(p1);
                ...
                $j(obj.box).find(' > div').append(html);
            ...
            function initEvents() {
                ...
                $j(window).on('resize', obj.tmp.events.resize);
            }
            ...
            function resizeStart(type, evnt) {
                ...
                if (!window.addEventListener) { window.document.attachEvent('onselectstart', function() { return false; } ); }
                $j(document).off('mousemove', obj.tmp.events.mouseMove).on('mousemove', obj.tmp.events.mouseMove);
                $j(document).off('mouseup', obj.tmp.events.mouseUp).on('mouseup', obj.tmp.events.mouseUp);
                obj.tmp.resize = {
                    type    : type,
                    x       : evnt.screenX,
                    y       : evnt.screenY,
                ...
                if (type == 'left' || type == 'right') {
                    obj.tmp.resize.value = parseInt($j('#layout_'+ obj.name +'_resizer_'+ type)[0].style.left);
                }
                if (type == 'top' || type == 'preview' || type == 'bottom') {
                    obj.tmp.resize.value = parseInt($j('#layout_'+ obj.name +'_resizer_'+ type)[0].style.top);
                }
            }
            ...
            function resizeStop(evnt) {
                ...
                if (!evnt) evnt = window.event;
                if (!window.addEventListener) { window.document.attachEvent('onselectstart', function() { return false; } ); }
                $j(document).off('mousemove', obj.tmp.events.mouseMove);
                $j(document).off('mouseup', obj.tmp.events.mouseUp);
                ...
                if (obj.tmp.diff_x !== 0 || obj.tmp.resize.diff_y !== 0) { // only recalculate if changed
                    ...
                    var height  = parseInt($j(obj.box).height());
                    var width   = parseInt($j(obj.box).width());
                ...
                $j('#layout_'+ obj.name + '_resizer_'+ obj.tmp.resize.type).removeClass('active');
            ...
            function resizeMove(evnt) {
                ...
                var p         = $j('#layout_'+ obj.name + '_resizer_'+ tmp.type);
        ...
        refresh: function (panel) {
            ...
            if (typeof panel == 'string') {
                ...
                $j(pname).css({ display: p.hidden ? 'none' : 'block' });
                if (p.resizable) $j(rname).show(); else $j(rname).hide();
                // insert content
                if (typeof p.content == 'object' && typeof p.content.render === 'function') {
                    p.content.box = $j(pname +'> .w2ui-panel-content')[0];
                    setTimeout(function () {
                        // need to remove unnecessary classes
                        if ($j(pname +'> .w2ui-panel-content').length > 0) {
                            $j(pname +'> .w2ui-panel-content')
                                .removeClass()
                                .removeAttr('name')
                ...
                var tmp = $j(obj.box).find(pname +'> .w2ui-panel-tabs');
                if (p.show.tabs) {
                ...
                tmp = $j(obj.box).find(pname +'> .w2ui-panel-toolbar');
                if (p.show.toolbar) {
                ...
                // show title
                tmp = $j(obj.box).find(pname +'> .w2ui-panel-title');
                if (p.title) {
                ...
                if ($j('#layout_'+ obj.name +'_panel_main').length == 0) {
                    obj.render();
                    return;
                }
                obj.resize();
        ...
        resize: function () {
            ...
            // layout itself
            var width  = parseInt($j(this.box).width());
            var height = parseInt($j(this.box).height());
            $j(this.box).find(' > div').css({
                width    : width + 'px',
                height    : height + 'px'
            });
            var obj = this;
            ...
            if (ptop !== null && ptop.hidden !== true) {
                ...
                $j('#layout_'+ this.name +'_panel_top').css({
                    'display': 'block',
                    'left': l + 'px',
                    'top': t + 'px',
                    'width': w + 'px',
                ...
                if (ptop.resizable) {
                    ...
                    $j('#layout_'+ this.name +'_resizer_top').show().css({
                        'display': 'block',
                        'left': l + 'px',
                        'top': t + 'px',
                        'width': w + 'px',
                ...
                $j('#layout_'+ this.name +'_panel_top').hide();
            }
            // left if any
            if (pleft !== null && pleft.hidden !== true) {
                ...
                e = $j('#layout_'+ this.name +'_panel_left');
                ...
                if (pleft.resizable) {
                    ...
                    $j('#layout_'+ this.name +'_resizer_left').show().css({
                        'display': 'block',
                        'left': l + 'px',
                        'top': t + 'px',
                        'width': w + 'px',
                ...
                $j('#layout_'+ this.name +'_panel_left').hide();
                $j('#layout_'+ this.name +'_resizer_left').hide();
            }
            // right if any
            if (pright !== null && pright.hidden !== true) {
                ...
                h = height - (stop ? ptop.sizeCalculated + this.padding : 0) -
                    (sbottom ? pbottom.sizeCalculated + this.padding : 0);
                $j('#layout_'+ this.name +'_panel_right').css({
                    'display': 'block',
                    'left': l + 'px',
                    'top': t + 'px',
                    'width': w + 'px',
                ...
                if (pright.resizable) {
                    l = l - this.padding;
                    w = (this.resizer > this.padding ? this.resizer : this.padding);
                    $j('#layout_'+ this.name +'_resizer_right').show().css({
                        'display': 'block',
                        'left': l + 'px',
                        'top': t + 'px',
                        'width': w + 'px',
                ...
                $j('#layout_'+ this.name +'_panel_right').hide();
            }
            // bottom if any
            if (pbottom !== null && pbottom.hidden !== true) {
                ...
                $j('#layout_'+ this.name +'_panel_bottom').css({
                    'display': 'block',
                    'left': l + 'px',
                    'top': t + 'px',
                    'width': w + 'px',
                ...
                if (pbottom.resizable) {
                    ...
                    h = (this.resizer > this.padding ? this.resizer : this.padding);
                    $j('#layout_'+ this.name +'_resizer_bottom').show().css({
                        'display': 'block',
                        'left': l + 'px',
                        'top': t + 'px',
                        'width': w + 'px',
                ...
                $j('#layout_'+ this.name +'_panel_bottom').hide();
            }
            // main - always there
            ...
            e = $j('#layout_'+ this.name +'_panel_main');
            if (window.navigator.userAgent.indexOf('MSIE') != -1 && e.length > 0 && e[0].clientHeight < e[0].scrollHeight) w += 17; // IE hack
            e.css({
                'display': 'block',
                'left': l + 'px',
            ...
            if (pprev !== null && pprev.hidden !== true) {
                ...
                e = $j('#layout_'+ this.name +'_panel_preview');
                if (window.navigator.userAgent.indexOf('MSIE') != -1 && e.length > 0 && e[0].clientHeight < e[0].scrollHeight) w += 17; // IE hack
                e.css({
                    'display': 'block',
                    'left': l + 'px',
                ...
                if (pprev.resizable) {
                    t = t - (this.padding === 0 ? 0 : this.padding);
                    h = (this.resizer > this.padding ? this.resizer : this.padding);
                    $j('#layout_'+ this.name +'_resizer_preview').show().css({
                        'display': 'block',
                        'left': l + 'px',
                        'top': t + 'px',
                        'width': w + 'px',
                ...
                $j('#layout_'+ this.name +'_panel_preview').hide();
            }
            ...
            for (var p1 in w2layout_panels) {
                ...
                if (pan) {
                    if (pan.title) {
                        tabHeight += w2utils.getSize($j(tmp2 + 'title').css({ top: tabHeight + 'px', display: 'block' }), 'height');
                    }
                    if (pan.show.tabs) {
                        if (pan.tabs !== null && w2ui[this.name +'_'+ p1 +'_tabs']) w2ui[this.name +'_'+ p1 +'_tabs'].resize();
                        tabHeight += w2utils.getSize($j(tmp2 + 'tabs').css({ top: tabHeight + 'px', display: 'block' }), 'height');
                    }
                    if (pan.show.toolbar) {
                        if (pan.toolbar !== null && w2ui[this.name +'_'+ p1 +'_toolbar']) w2ui[this.name +'_'+ p1 +'_toolbar'].resize();
                        tabHeight += w2utils.getSize($j(tmp2 + 'toolbar').css({ top: tabHeight + 'px', display: 'block' }), 'height');
                    }
                }
                $j(tmp2 + 'content').css({ display: 'block' }).css({ top: tabHeight + 'px' });
            }
            ...
            this._resize_timer = setTimeout(function () {
                for (var e in w2ui) {
                    if (typeof w2ui[e].resize == 'function') {
                        ...
                        var parent = $j(w2ui[e].box).parents('.w2ui-layout');
        ...
        destroy: function () {
            ...
            // clean up
            if ($j(this.box).find('#layout_'+ this.name +'_panel_main').length > 0) {
                $j(this.box)
                    .removeAttr('name')
                    .removeClass('w2ui-layout')
                    .html('');
            ...
            // event after
            this.trigger($j.extend(eventData, { phase: 'after' }));
            if (this.tmp.events && this.tmp.events.resize) $j(window).off('resize', this.tmp.events.resize);
            return true;
        },
    ...
    $j.extend(w2layout.prototype, w2utils.event);
...
(function () {
    ...
    $j.fn.w2popup = function(method, options) {
        if (typeof method === 'undefined') {
            options = {};
            method  = 'open';
        }
        if ($j.isPlainObject(method)) {
            options = method;
            method  = 'open';
        }
        method = method.toLowerCase();
        if (method === 'load' && typeof options === 'string') {
            options = $j.extend({ url: options }, arguments.length > 2 ? arguments[2] : {});
        }
        ...
        var dlgOptions = {};
        if ($j(this).length > 0) {
            if ($j(this).find('div[rel=title], div[rel=body], div[rel=buttons]').length > 0) {
                if ($j(this).find('div[rel=title]').length > 0) {
                    dlgOptions['title'] = $j(this).find('div[rel=title]').html();
                }
                if ($j(this).find('div[rel=body]').length > 0) {
                    dlgOptions['body']  = $j(this).find('div[rel=body]').html();
                    dlgOptions['style'] = $j(this).find('div[rel=body]')[0].style.cssText;
                }
                if ($j(this).find('div[rel=buttons]').length > 0) {
                    dlgOptions['buttons'] = $j(this).find('div[rel=buttons]').html();
                }
            } else {
                dlgOptions['title'] = '&nbsp;';
                dlgOptions['body']  = $j(this).html();
            }
            if (parseInt($j(this).css('width')) != 0)  dlgOptions['width']  = parseInt($j(this).css('width'));
            if (parseInt($j(this).css('height')) != 0) dlgOptions['height'] = parseInt($j(this).css('height'));
        }
        // show popup
        return w2popup[method]($j.extend({}, dlgOptions, options));
    };
    ...
    w2popup = {
        ...
        open: function (options) {
            ...
            var old_options = $j('#w2ui-popup').data('options');
            var options = $j.extend({}, this.defaults, old_options, { title: '', body : '', buttons: '' }, options, { maximized: false });
            // need timer because popup might not be open
            setTimeout(function () { $j('#w2ui-popup').data('options', options); }, 100);
            // if new - reset event handlers
            if ($j('#w2ui-popup').length == 0) {
                w2popup.handlers  = [];
                w2popup.onMax     = null;
                w2popup.onMin     = null;
            ...
            if ($j('#w2ui-popup').length == 0) {
                ...
                // show new items
                var body = $j('#w2ui-popup .w2ui-box2 > .w2ui-msg-body').html(options.body);
                if (body.length > 0) body[0].style.cssText = options.style;
                if (options.buttons != '') {
                    $j('#w2ui-popup .w2ui-msg-buttons').show().html(options.buttons);
                    $j('#w2ui-popup .w2ui-msg-body').removeClass('w2ui-msg-no-buttons');
                    $j('#w2ui-popup .w2ui-box1, #w2ui-popup .w2ui-box2').css('bottom', '');
                } else {
                    $j('#w2ui-popup .w2ui-msg-buttons').hide().html('');
                    $j('#w2ui-popup .w2ui-msg-body').addClass('w2ui-msg-no-buttons');
                    $j('#w2ui-popup .w2ui-box1, #w2ui-popup .w2ui-box2').css('bottom', '0px');
                }
                if (options.title != '') {
                    $j('#w2ui-popup .w2ui-msg-title').show().html(
                          ...
                          options.title);
                    $j('#w2ui-popup .w2ui-msg-body').removeClass('w2ui-msg-no-title');
                    $j('#w2ui-popup .w2ui-box1, #w2ui-popup .w2ui-box2').css('top', '');
                } else {
                    $j('#w2ui-popup .w2ui-msg-title').hide().html('');
                    $j('#w2ui-popup .w2ui-msg-body').addClass('w2ui-msg-no-title');
                    $j('#w2ui-popup .w2ui-box1, #w2ui-popup .w2ui-box2').css('top', '0px');
                }
                // transition
                var div_old = $j('#w2ui-popup .w2ui-box1')[0];
                var div_new = $j('#w2ui-popup .w2ui-box2')[0];
                w2utils.transition(div_old, div_new, options.transition);
                div_new.className = 'w2ui-box1';
                div_old.className = 'w2ui-box2';
                $j(div_new).addClass('w2ui-current-box');
                // remove max state
                $j('#w2ui-popup').data('prev-size', null);
                // call event onChange
                setTimeout(function () {
                    w2popup.status = 'open';
                    obj.trigger($j.extend(eventData, { phase: 'after' }));
            ...
            // keyboard events
            if (options.keyboard) $j(document).on('keydown', this.keydown);
            ...
            $j('#w2ui-popup .w2ui-msg-title').on('mousedown', function (event) { mvStart(event); })
            ...
            function mvStart(evnt) {
                ...
                tmp.x = evnt.screenX;
                tmp.y = evnt.screenY;
                tmp.pos_x = $j('#w2ui-popup').position().left;
                tmp.pos_y = $j('#w2ui-popup').position().top;
                w2popup.lock({ opacity: 0 });
                $j(document).on('mousemove', tmp.mvMove);
                $j(document).on('mouseup', tmp.mvStop);
            ...
            function mvStop(evnt) {
                ...
                tmp.resizing = false;
                $j(document).off('mousemove', tmp.mvMove);
                $j(document).off('mouseup', tmp.mvStop);
                w2popup.unlock();
            }
            return this;
        },
        ...
        keydown: function (event) {
            var options = $j('#w2ui-popup').data('options');
            ...
            switch (event.keyCode) {
                case 27:
                    ...
                    if ($j('#w2ui-popup .w2ui-popup-message').length > 0) w2popup.message(); else w2popup.close();
                    break;
            }
            // event after
            w2popup.trigger($j.extend(eventData, { phase: 'after'}));
        },

        close: function (options) {
            var obj = this;
            var options = $j.extend({}, $j('#w2ui-popup').data('options'), options);
            if ($j('#w2ui-popup').length == 0) return;
            // trigger event
            ...
            if (options.keyboard) $j(document).off('keydown', this.keydown);
        },
        ...
        max: function () {
            ...
            options.prevSize = $j('#w2ui-popup').css('width') + ':' + $j('#w2ui-popup').css('height');
            // do resize
        ...
        get: function () {
            return $j('#w2ui-popup').data('options');
        },
        ...
        clear: function() {
            $j('#w2ui-popup .w2ui-msg-title').html('');
            $j('#w2ui-popup .w2ui-msg-body').html('');
            $j('#w2ui-popup .w2ui-msg-buttons').html('');
        },
        ...
        load: function (options) {
            ...
            // load url
            var html = $j('#w2ui-popup').data(url);
            if (typeof html != 'undefined' && html != null) {
                popup(html, selector);
            } else {
                $j.get(url, function (data, status, obj) { // should always be $j.get as it is template
                    popup(obj.responseText, selector);
                    $j('#w2ui-popup').data(url, obj.responseText); // remember for possible future purposes
                });
            }
            function popup(html, selector) {
                delete options.url;
                $j('body').append('<div id="w2ui-tmp" style="display: none">' + html + '</div>');
                if (typeof selector != 'undefined' && $j('#w2ui-tmp #'+selector).length > 0) {
                    $j('#w2ui-tmp #' + selector).w2popup(options);
                } else {
                    $j('#w2ui-tmp > div').w2popup(options);
                }
                // link styles
                if ($j('#w2ui-tmp > style').length > 0) {
                    var style = $j('<div>').append($j('#w2ui-tmp > style').clone()).html();
                    if ($j('#w2ui-popup #div-style').length == 0) {
                        $j('#w2ui-popup').append('<div id="div-style" style="position: absolute; left: -100; width: 1px"></div>');
                    }
                    $j('#w2ui-popup #div-style').html(style);
                }
                $j('#w2ui-tmp').remove();
            }
        },
        ...
        message: function (options) {
            ...
            var poptions = $j('#w2ui-popup').data('options') || {};
            ...
            var head     = $j('#w2ui-popup .w2ui-msg-title');
            var pwidth   = parseInt($j('#w2ui-popup').width());
            var msgCount = $j('#w2ui-popup .w2ui-popup-message').length;
            // remove message
            if ($j.trim(options.html) == '') {
                $j('#w2ui-popup #w2ui-message'+ (msgCount-1)).css('z-Index', 250);
                var options = $j('#w2ui-popup #w2ui-message'+ (msgCount-1)).data('options') || {};
                $j('#w2ui-popup #w2ui-message'+ (msgCount-1)).remove();
                if (typeof options.onClose == 'function') options.onClose();
                if (msgCount == 1) {
                    w2popup.unlock();
                } else {
                    $j('#w2ui-popup #w2ui-message'+ (msgCount-2)).show();
                }
            } else {
                // hide previous messages
                $j('#w2ui-popup .w2ui-popup-message').hide();
                // add message
                $j('#w2ui-popup .w2ui-box1')
                    .before('<div id="w2ui-message' + msgCount + '" class="w2ui-popup-message" style="display: none; ' +
                            ...
                            '</div>');
                $j('#w2ui-popup #w2ui-message'+ msgCount).data('options', options);
                var display = $j('#w2ui-popup #w2ui-message'+ msgCount).css('display');
                $j('#w2ui-popup #w2ui-message'+ msgCount).css({
                ...
                if (display == 'none') {
                    $j('#w2ui-popup #w2ui-message'+ msgCount).show().html(options.html);
        ...
        lock: function (msg, showSpinner) {
            var args = Array.prototype.slice.call(arguments, 0);
            args.unshift($j('#w2ui-popup'));
            w2utils.lock.apply(window, args);
        },

        unlock: function () {
            w2utils.unlock($j('#w2ui-popup'));
        },

        // --- INTERNAL FUNCTIONS

        lockScreen: function (options) {
            if ($j('#w2ui-lock').length > 0) return false;
            if (typeof options == 'undefined') options = $j('#w2ui-popup').data('options');
            if (typeof options == 'undefined') options = {};
            options = $j.extend({}, w2popup.defaults, options);
            // show element
            $j('body').append('<div id="w2ui-lock" ' +
            ...
            if (options.modal == true) {
                $j('#w2ui-lock').on('mousedown', function () {
                    $j('#w2ui-lock').css({
                ...
                $j('#w2ui-lock').on('mouseup', function () {
                    setTimeout(function () {
                        $j('#w2ui-lock').css({
                            '-webkit-transition': '.1s',
                            '-moz-transition': '.1s',
                ...
                $j('#w2ui-lock').on('mouseup', function () { w2popup.close(); });
            }
            return true;
        },
        ...
        resize: function (width, height, callBack) {
            ...
            if (parseInt($j(window).width())  - 10 < parseInt(width))  width  = parseInt($j(window).width())  - 10;
            if (parseInt($j(window).height()) - 10 < parseInt(height)) height = parseInt($j(window).height()) - 10;
            var top  = ((parseInt($j(window).height()) - parseInt(height)) / 2) * 0.8;
            var left = (parseInt($j(window).width()) - parseInt(width)) / 2;
            // resize there
            $j('#w2ui-popup').css({
    ...
    $j.extend(w2popup, w2utils.event);
...
var w2alert = function (msg, title, callBack) {
    ...
    if ($j('#w2ui-popup').length > 0 && w2popup.status != 'closing') {
        w2popup.message({
            width   : 400,
            height  : 170,
...
var w2confirm = function (msg, title, callBack) {
    ...
    var defaults = {
        ...
        width       : ($j('#w2ui-popup').length > 0 ? 400 : 450),
        height      : ($j('#w2ui-popup').length > 0 ? 170 : 220),
        yes_text    : 'Yes',
        yes_class   : '',
        yes_style   : '',
        yes_callBack: null,
    ...
    if (arguments.length == 1 && typeof msg == 'object') {
        $j.extend(options, defaults, msg);
    } else {
        if (typeof title == 'function') {
            $j.extend(options, defaults, {
                msg     : msg,
                callBack: title
            })            
        } else {
        ...
        if (options.height > (w2popup.get().height - 50)) options.height = w2popup.get().height - 50;
          w2popup.message({
            ...
            html    : '<div style="position: absolute; top: 0px; left: 0px; right: 0px; bottom: 40px; overflow: auto">' +
                      ...
                      '</div>',
            onOpen: function () {
                $j('#w2ui-popup .w2ui-popup-message .btn').on('click', function (event) {
                    w2popup.message();
        ...
        w2popup.open({
            ...
            onOpen: function (event) {
                event.onComplete = function () {
                    $j('#w2ui-popup .w2ui-popup-btn').on('click', function (event) {
                        w2popup.close();
                        if (typeof options.callBack == 'function') options.callBack(event.target.id);
                        if (event.target.id == 'Yes' && typeof options.yes_callBack == 'function') options.yes_callBack();
                        if (event.target.id == 'No'  && typeof options.no_callBack == 'function') options.no_callBack();
...
(function () {
    var w2tabs = function (options) {
        ...
        $j.extend(this, { handlers: [] });
        $j.extend(true, this, w2obj.tabs, options);
    };

    // ====================================================
    // -- Registers as a jQuery plugin

    $j.fn.w2tabs = function(method) {
        if (typeof method === 'object' || !method ) {
            // check name parameter
            ...
            for (var i = 0; i < tabs.length; i++) {
                object.tabs[i] = $j.extend({}, w2tabs.prototype.tab, tabs[i]);
            }
            if ($j(this).length !== 0) {
                object.render($j(this)[0]);
            }
    ...
    w2tabs.prototype = {
        ...
        insert: function (id, tab) {
            ...
            for (var i = 0; i < tab.length; i++) {
                ...
                // add tab
                var newTab = $j.extend({}, w2tabs.prototype.tab, tab[i]);
        ...
        remove: function () {
            ...
            for (var a = 0; a < arguments.length; a++) {
                ...
                // remove from screen
                $j(this.box).find('#tabs_'+ this.name +'_tab_'+ w2utils.escapeId(tab.id)).remove();
            }
            return removed;
        },
        ...
        refresh: function (id) {
            ...
            if (typeof id === 'undefined') {
                ...
                var jq_el   = $j(this.box).find('#tabs_'+ this.name +'_tab_'+ w2utils.escapeId(tab.id));
                ...
                if (jq_el.length === 0) {
                    ...
                    if (this.get(id, true) !== this.tabs.length-1 && $j(this.box).find('#tabs_'+ this.name +'_tab_'+ w2utils.escapeId(this.tabs[parseInt(this.get(id, true))+1].id)).length > 0) {
                        $j(this.box).find('#tabs_'+ this.name +'_tab_'+ w2utils.escapeId(this.tabs[parseInt(this.get(id, true))+1].id)).before(html);
                    } else {
                        $j(this.box).find('#tabs_'+ this.name +'_right').before(html);
                    }
                } else {
                    // refresh
                    jq_el.html(tabHTML);
            ...
            // right html
            $j('#tabs_'+ this.name +'_right').html(this.right);
            // event after
            this.trigger($j.extend(eventData, { phase: 'after' }));
            return (new Date()).getTime() - time;
        },
        ...
        render: function (box) {
            ...
            // if (window.getSelection) window.getSelection().removeAllRanges(); // clear selection
            if (typeof box !== 'undefined' && box !== null) {
                if ($j(this.box).find('> table #tabs_'+ this.name + '_right').length > 0) {
                    $j(this.box)
                        .removeAttr('name')
        ...
        click: function (id, event) {
            ...
            if (eventData.isCancelled === true) return;
            // default action
            $j(this.box).find('#tabs_'+ this.name +'_tab_'+ w2utils.escapeId(this.active) +' .w2ui-tab').removeClass('active');
            this.active = tab.id;
            // route processing
            if (tab.route) {
        ...
        animateClose: function(id, event) {
            ...
            // default action
            var obj = this;
            $j(this.box).find('#tabs_'+ this.name +'_tab_'+ w2utils.escapeId(tab.id)).css({
                '-webkit-transition': '.2s',
                '-moz-transition': '2s',
                '-ms-transition': '.2s',
                '-o-transition': '.2s',
                opacity: '0' });
            setTimeout(function () {
                var width = $j(obj.box).find('#tabs_'+ obj.name +'_tab_'+ w2utils.escapeId(tab.id)).width();
                $j(obj.box).find('#tabs_'+ obj.name +'_tab_'+ w2utils.escapeId(tab.id))
                    .html('<div style="width: '+ width +'px; -webkit-transition: .2s; -moz-transition: .2s; -ms-transition: .2s; -o-transition: .2s"></div>');
                setTimeout(function () {
                    $j(obj.box).find('#tabs_'+ obj.name +'_tab_'+ w2utils.escapeId(tab.id)).find(':first-child').css({ 'width': '0px' });
                }, 50);
            }, 200);
            setTimeout(function () {
                obj.remove(id);
        ...
        animateInsert: function(id, tab) {
            ...
            if (tab.disabled) { addStyle += 'opacity: 0.2; -moz-opacity: 0.2; -webkit-opacity: 0.2; -o-opacity: 0.2; filter:alpha(opacity=20);'; }
            var html = '<td id="tabs_'+ this.name +'_tab_'+ tab.id +'" style="'+ addStyle +'" valign="middle">'+ tabHTML +'</td>';
            if (this.get(id, true) !== this.tabs.length && $j(this.box).find('#tabs_'+ this.name +'_tab_'+ w2utils.escapeId(this.tabs[parseInt(this.get(id, true))].id)).length > 0) {
                $j(this.box).find('#tabs_'+ this.name +'_tab_'+ w2utils.escapeId(this.tabs[parseInt(this.get(id, true))].id)).before(html);
            } else {
                $j(this.box).find('#tabs_'+ this.name +'_right').before(html);
            }
            // -- move
            var obj = this;
            setTimeout(function () {
                var width = $j('#_tmp_simple_tab').width();
                $j('#_tmp_tabs').remove();
                $j('#tabs_'+ obj.name +'_tab_'+ w2utils.escapeId(tab.id) +' > div').css('width', width+'px');
            }, 1);
            setTimeout(function () {
                // insert for real
                obj.insert(id, tab);
    ...
    $j.extend(w2tabs.prototype, w2utils.event);
    w2obj.tabs = w2tabs;
})();
...
(function () {
    var w2toolbar = function (options) {
        ...
        $j.extend(true, this, w2obj.toolbar, options);
    };

    // ====================================================
    // -- Registers as a jQuery plugin

    $j.fn.w2toolbar = function(method) {
        if (typeof method === 'object' || !method ) {
            // check name parameter
            ...
            $j.extend(object, { items: [], handlers: [] });
            for (var i = 0; i < items.length; i++) {
                object.items[i] = $j.extend({}, w2toolbar.prototype.item, items[i]);
            }
            if ($j(this).length !== 0) {
                object.render($j(this)[0]);
            }
    ...
    w2toolbar.prototype = {
        ...
        insert: function (id, items) {
            ...
            for (var o = 0; o < items.length; o++) {
                ...
                if ($j.inArray(String(items[o].type), ['button', 'check', 'radio', 'drop', 'menu', 'break', 'html', 'spacer']) === -1) {
                ...
                // add item
                var it = $j.extend({}, w2toolbar.prototype.item, items[o]);
                if (id == null) {
                    this.items.push(it);
                } else {
                    var middle = this.get(id, true);
        ...
        remove: function () {
            ...
            for (var a = 0; a < arguments.length; a++) {
                ...
                removed++;
                // remove from screen
                $j(this.box).find('#tb_'+ this.name +'_item_'+ w2utils.escapeId(it.id)).remove();
                // remove from array
        ...
        set: function (id, item) {
            var index = this.get(id, true);
            if (index === null) return false;
            $j.extend(this.items[index], item);
            this.refresh(id);
            return true;
        },
        ...
        render: function (box) {
            ...
            if (box != null) {
                if ($j(this.box).find('> table #tb_'+ this.name + '_right').length > 0) {
                    $j(this.box)
                        .removeAttr('name')
        ...
        refresh: function (id) {
            ...
            var el = $j(this.box).find('#tb_'+ this.name +'_item_'+ w2utils.escapeId(it.id));
            ...
            if (el.length === 0) {
                ...
                if (this.get(id, true) === this.items.length-1) {
                    $j(this.box).find('#tb_'+ this.name +'_right').before(html);
                } else {
                    $j(this.box).find('#tb_'+ this.name +'_item_'+ w2utils.escapeId(this.items[parseInt(this.get(id, true))+1].id)).before(html);
                }
            } else {
                // refresh
                el.html(html);
            ...
            switch (item.type) {
                ...
                case 'drop':
                    ...
                    html += '<table cellpadding="0" cellspacing="0" title="'+ item.hint +'" class="w2ui-button '+ (item.checked ? 'checked' : '') +'" '+
                            ...
                            '       onmouseover = "' + (!item.disabled ? "$j(this).addClass('over');" : "") + '"'+
                            '       onmouseout  = "' + (!item.disabled ? "$j(this).removeClass('over').removeClass('down');" : "") + '"'+
                            '       onmousedown = "' + (!item.disabled ? "$j(this).addClass('down');" : "") + '"'+
                            '       onmouseup   = "' + (!item.disabled ? "$j(this).removeClass('down');" : "") + '"'+
                            '>'+
                            '<tr><td>'+
        ...
        click: function (id, event) {
            ...
            if (it && !it.disabled) {
                ...
                var btn = $j('#tb_'+ this.name +'_item_'+ w2utils.escapeId(it.id) +' table.w2ui-button');
                ...
                if (it.type === 'drop' || it.type === 'menu') {
                    if (it.checked) {
                        ...
                        // show overlay
                        setTimeout(function () {
                            var el = $j('#tb_'+ obj.name +'_item_'+ w2utils.escapeId(it.id));
                            if (!$j.isPlainObject(it.overlay)) it.overlay = {};
                            var left = (el.width() - 50) / 2;
                            if (left > 19) left = 19;
                            if (it.type === 'drop') {
                                el.w2overlay(it.html, $j.extend({ left: left, top: 3 }, it.overlay));
                            }
                            if (it.type === 'menu') {
                                el.w2menu(it.items, $j.extend({ left: left, top: 3 }, it.overlay, {
                            ...
                            $j(document).on('click', hideDrop);
                            function hideDrop() {
                                $j(document).off('click', hideDrop);
                                it.checked = false;
    ...
    $j.extend(w2toolbar.prototype, w2utils.event);
...
(function () {
    var w2sidebar = function (options) {
        ...
        $j.extend(true, this, w2obj.sidebar, options);
    };

    // ====================================================
    // -- Registers as a jQuery plugin

    $j.fn.w2sidebar = function(method) {
        if (typeof method === 'object' || !method ) {
            // check name parameter
            ...
            $j.extend(object, { handlers: [], nodes: [] });
    ...
    w2sidebar.prototype = {
        ...
        insert: function (parent, before, nodes) {
            ...
            if (arguments.length == 2) {
                ...
                ind    = this.get(before);
                if (ind === null) {
                    if (!$j.isArray(nodes)) nodes = [nodes];
            ...
            for (var o in nodes) {
                ...
                tmp = $j.extend({}, w2sidebar.prototype.node, node);
                tmp.sidebar = this;
                tmp.parent  = parent;
                nd = tmp.nodes || [];
        ...
        set: function (parent, id, node) {
            ...
            for (var i = 0; i < parent.nodes.length; i++) {
                if (parent.nodes[i].id === id) {
                    ...
                    $j.extend(parent.nodes[i], node, { nodes: [] });
                    if (nodes != null) {
        ...
        select: function (id) {
            ...
            $j(this.box).find('#node_'+ w2utils.escapeId(id))
        ...
        collapse: function (id) {
            ...
            if (eventData.isCancelled === true) return;
            // default action
            $j(this.box).find('#node_'+ w2utils.escapeId(id) +'_sub').slideUp(200);
            $j(this.box).find('#node_'+ w2utils.escapeId(id) +' .w2ui-node-dots:first-child').html('<div class="w2ui-expand">+</div>');
            nd.expanded = false;
            // event after
            this.trigger($j.extend(eventData, { phase: 'after' }));
        ...
        expand: function (id) {
            ...
            if (eventData.isCancelled === true) return;
            // default action
            $j(this.box).find('#node_'+ w2utils.escapeId(id) +'_sub').slideDown(200);
            $j(this.box).find('#node_'+ w2utils.escapeId(id) +' .w2ui-node-dots:first-child').html('<div class="w2ui-expand">-</div>');
            nd.expanded = true;
            // event after
            this.trigger($j.extend(eventData, { phase: 'after' }));
        ...
        click: function (id, event) {
            ...
            $j(obj.box).find('.w2ui-node.w2ui-selected').each(function (index, el) {
                var oldID     = $j(el).attr('id').replace('node_', '');
                var oldNode = obj.get(oldID);
                if (oldNode != null) oldNode.selected = false;
                $j(el).removeClass('w2ui-selected').find('.w2ui-icon').removeClass('w2ui-icon-selected');
            });
            // select new one
            var newNode = $j(obj.box).find('#node_'+ w2utils.escapeId(id));
            var oldNode = $j(obj.box).find('#node_'+ w2utils.escapeId(obj.selected));
        ...
        keydown: function (event) {
            ...
            if ($j.inArray(event.keyCode, [13, 32, 37, 38, 39, 40]) != -1) {
                if (event.preventDefault) event.preventDefault();
        ...
        scrollIntoView: function (id) {
            ...
            var nd = this.get(id);
            if (nd === null) return;
            var body   = $j(this.box).find('.w2ui-sidebar-div');
            var item   = $j(this.box).find('#node_'+ w2utils.escapeId(id));
        ...
        contextMenu: function (id, event) {
            ...
            setTimeout(function () {
                ...
                if (obj.menu.length > 0) {
                    $j(obj.box).find('#node_'+ w2utils.escapeId(id))
                        .w2menu(obj.menu, {
        ...
        render: function (box) {
            ...
            // default action
            if (typeof box != 'undefined' && box !== null) {
                if ($j(this.box).find('> div > div.w2ui-sidebar-div').length > 0) {
                    $j(this.box)
                        .removeAttr('name')
            ...
            $j(this.box).find('> div').css({
                width    : $j(this.box).width() + 'px',
                height: $j(this.box).height() + 'px'
            });
            if ($j(this.box).length > 0) $j(this.box)[0].style.cssText += this.style;
            // adjust top and bottom
            if (this.topHTML !== '') {
                $j(this.box).find('.w2ui-sidebar-top').html(this.topHTML);
                $j(this.box).find('.w2ui-sidebar-div')
                    .css('top', $j(this.box).find('.w2ui-sidebar-top').height() + 'px');
            }
            if (this.bottomHTML !== '') {
                $j(this.box).find('.w2ui-sidebar-bottom').html(this.bottomHTML);
                $j(this.box).find('.w2ui-sidebar-div')
                    .css('bottom', $j(this.box).find('.w2ui-sidebar-bottom').height() + 'px');
            }
            // event after
            this.trigger($j.extend(eventData, { phase: 'after' }));
            // ---
        ...
        refresh: function (id) {
            ...
            if (node !== this) {
                ...
                $j(this.box).find(tmp).before('<div id="sidebar_'+ this.name + '_tmp"></div>');
                $j(this.box).find(tmp).remove();
                $j(this.box).find(nm).remove();
                $j('#sidebar_'+ this.name + '_tmp').before(nodeHTML);
                $j('#sidebar_'+ this.name + '_tmp').remove();
            }
            // refresh sub nodes
            $j(this.box).find(nm).html('');
            for (var i = 0; i < node.nodes.length; i++) {
                nd = node.nodes[i];
                nodeHTML = getNodeHTML(nd);
                $j(this.box).find(nm).append(nodeHTML);
            ...
            function getNodeHTML(nd) {
                ...
                if (nd.group) {
                    html =
                        ...
                        '        onmouseout="$j(this).find(\'span:nth-child(1)\').css(\'color\', \'transparent\')" '+
                        '        onmouseover="$j(this).find(\'span:nth-child(1)\').css(\'color\', \'inherit\')">'+
        ...
        resize: function () {
            ...
            if (eventData.isCancelled === true) return;
            // default action
            $j(this.box).css('overflow', 'hidden');    // container should have no overflow
            //$j(this.box).find('.w2ui-sidebar-div').css('overflow', 'hidden');
            $j(this.box).find('> div').css({
                width        : $j(this.box).width() + 'px',
                height    : $j(this.box).height() + 'px'
            });
            //$j(this.box).find('.w2ui-sidebar-div').css('overflow', 'auto');
            // event after
            this.trigger($j.extend(eventData, { phase: 'after' }));
            return (new Date()).getTime() - time;
        },
    ...
    $j.extend(w2sidebar.prototype, w2utils.event);
...
(function ($) {
    ...
    var w2field = function (options) {
        ...
        this.options     = $j.extend(true, {}, options);
        ...
        $j.extend(true, this, w2obj.field);
    };

    // ====================================================
    // -- Registers as a jQuery plugin

    $j.fn.w2field = function (method, options) {
        // call direct
        if (this.length == 0) {
            ...
            if (arguments.length == 0) {
                var obj = $j(this).data('w2field');
                return obj;
            }
            if (typeof method == 'string' && typeof options == 'object') {
                method = $j.extend(true, {}, options, { type: method });
            }
            ...
            return this.each(function (index, el) {
                var obj = $j(el).data('w2field');
                // if object is not defined, define it
                if (typeof obj == 'undefined') {
                    var obj = new w2field(method);
                    $j.extend(obj, { handlers: [] });
                    if (el) obj.el = $j(el)[0];
                    obj.init();
                    $j(el).data('w2field', obj);
                    return obj;
                } else { // fully re-init
                    obj.clear();
    ...
    /*     To add custom types
        $j().w2field('addType', 'myType', function (options) {
            $j(this.el).on('keypress', function (event) {
            ...
            $j(this.el).on('blur', function (event)  { // keyCode & charCode differ in FireFox
                var ch = this.value;
                if (ch != 'a' && ch != 'b' && ch != 'c') {
                    $j(this).w2tag(w2utils.lang("Not a single charecter from the set of 'abc'"));
                }
            });
        });
    */
    ...
    w2field.prototype = {
        ...
        init: function () {
            ...
            switch (this.type) {
                ...
                case 'hex':
                    ...
                    this.addSuffix();
                    if ($j(this.el).attr('placeholder') && options.placeholder == '') options.placeholder = $j(this.el).attr('placeholder');
                    $j(this.el).attr('placeholder', options.placeholder);
                    break;

                case 'color':
                    defaults = {
                        prefix      : '#',
                        suffix      : '<div style="width: '+ (parseInt($j(this.el).css('font-size')) || 12) +'px">&nbsp;</div>',
                        placeholder : '',
                        arrows      : false,
                        keyboard    : false
                    };
                    ...
                    // additional checks
                    $j(this.el).attr('maxlength', 6);
                    if ($j(this.el).val() != '') setTimeout(function () { $j(obj.el).change(); }, 1);
                    if ($j(this.el).attr('placeholder') && options.placeholder == '') options.placeholder = $j(this.el).attr('placeholder');
                    $j(this.el).attr('placeholder', options.placeholder);
                    break;
                ...
                case 'date':
                    ...
                    options = this.options; // since object is re-created, need to re-assign
                    if ($j(this.el).attr('placeholder') && options.placeholder == '') options.placeholder = $j(this.el).attr('placeholder');
                    $j(this.el).attr('placeholder', options.placeholder ? options.placeholder : options.format);
                    break;
                ...
                case 'time':
                    ...
                    options = this.options; // since object is re-created, need to re-assign
                    if ($j(this.el).attr('placeholder') && options.placeholder == '') options.placeholder = $j(this.el).attr('placeholder');
                    $j(this.el).attr('placeholder', options.placeholder ? options.placeholder : (options.format == 'h12' ? 'hh:mi pm' : 'hh:mi'));
                    break;
                ...
                case 'combo':
                    ...
                    if (this.type == 'list') {
                        ...
                        defaults.suffix = '<div class="arrow-down" style="margin-top: '+ ((parseInt($j(this.el).height()) - 6) / 2) +'px;"></div>';
                        $j(this.el).addClass('w2ui-select');
                        // if simple value - look it up
                        if (!$j.isPlainObject(options.selected)) {
                            for (var i in options.items) {
                                var item = options.items[i];
                                if (item && item.id == options.selected) {
                                    options.selected = $j.extend(true, {}, item);
                                    break;
                                }
                            }
                        }
                    ...
                    if (!$j.isPlainObject(options.selected)) options.selected = {};
                    $j(this.el).data('selected', options.selected);
                    ...
                    if ($j(this.el).attr('placeholder') && options.placeholder == '') options.placeholder = $j(this.el).attr('placeholder');
                    $j(this.el).attr('placeholder', options.placeholder).attr('autocomplete', 'off');
                    if (typeof options.selected.text != 'undefined') $j(this.el).val(options.selected.text);
                    break;
                ...
                case 'enum':
                    ...
                    this.options = options;
                    if (!$j.isArray(options.selected)) options.selected = [];
                    $j(this.el).data('selected', options.selected);
                    if (options.url) this.request(0);
                    this.addSuffix();
                    this.addMulti();
                ...
                case 'file':
                    ...
                    if (!$j.isArray(options.selected)) options.selected = [];
                    $j(this.el).data('selected', options.selected);
                    if ($j(this.el).attr('placeholder')) options.placeholder = $j(this.el).attr('placeholder');
                    this.addMulti();
                    break;
            }
            // attach events
        ...
        clear: function () {
            ...
            if (['money', 'currency'].indexOf(this.type) != -1) {
                $j(this.el).val($j(this.el).val().replace(options.moneyRE, ''));
            }
            if (this.type == 'percent') {
                $j(this.el).val($j(this.el).val().replace(/%/g, ''));
            }
            if (this.type == 'color') {
                $j(this.el).removeAttr('maxlength');
            }
            if (this.type == 'list') {
                $j(this.el).removeClass('w2ui-select');
            }
            if (['date', 'time'].indexOf(this.type) != -1) {
                if ($j(this.el).attr('placeholder') == options.format) $j(this.el).attr('placeholder', '');
            }
            this.type = 'clear';
            var tmp = $j(this.el).data('tmp');
            if (!this.tmp) return;
            // restore paddings
            if (typeof tmp != 'undefined') {
                if (tmp && tmp['old-padding-left'])  $j(this.el).css('padding-left',  tmp['old-padding-left']);
                if (tmp && tmp['old-padding-right']) $j(this.el).css('padding-right', tmp['old-padding-right']);
            }
            // remove events and data
            $j(this.el)
                .val(this.clean($j(this.el).val()))
                .removeClass('w2field')
            ...
            // remove helpers
            for (var h in this.helpers) $j(this.helpers[h]).remove();
            this.helpers = {};
        },
        ...
        refresh: function () {
            var obj       = this;
            var options   = this.options;
            var selected  = $j(this.el).data('selected');
            var time      = (new Date()).getTime();
            // enum
            if (['list'].indexOf(this.type) != -1) {
                $j(obj.el).parent().css('white-space', 'nowrap'); // needs this for arrow alway to appear on the right side
                // hide focus and show text
                if (obj.helpers.prefix) obj.helpers.prefix.hide();
                setTimeout(function () {
                    if (!obj.helpers.focus) return;
                    // if empty show no icon
                    if (!$j.isEmptyObject(selected) && options.icon) {
                    ...
                    if ($j(focus).val() == '') {
                        $j(focus).css('opacity', 0).prev().css('opacity', 0);
                        $j(obj.el).val(selected && selected.text != null ? selected.text : '');
                        $j(obj.el).attr('placeholder', options.placeholder || '');
                    } else {
                        $j(focus).css('opacity', 1).prev().css('opacity', 1);
                        $j(obj.el).val('');
                        $j(obj.el).removeAttr('placeholder');
                        setTimeout(function () {
                            if (obj.helpers.prefix) obj.helpers.prefix.hide(); 
                            var tmp = 'position: absolute; opacity: 0; margin: 4px 0px 0px 2px; background-position: left !important;';
                            if (options.icon) {
                                $j(focus).css('margin-left', '17px');
                                $j(obj.helpers.focus).find('.icon-search').attr('style', tmp + 'width: 11px !important; opacity: 1');
                            } else {
                                $j(focus).css('margin-left', '0px');
                                $j(obj.helpers.focus).find('.icon-search').attr('style', tmp + 'width: 0px !important; opacity: 0');
                            }
                        }, 1);
                    }
                    // if readonly or disabled
                    if ($j(obj.el).prop('readonly') || $j(obj.el).prop('disabled')) {
                        setTimeout(function () { 
                            $j(obj.helpers.prefix).css('opacity', '0.6'); 
                            $j(obj.helpers.suffix).css('opacity', '0.6');
                        }, 1);
                    } else {
                        setTimeout(function () { 
                            $j(obj.helpers.prefix).css('opacity', '1'); 
                            $j(obj.helpers.suffix).css('opacity', '1');
                        }, 1);
                    }
                }, 1);
            }
            if (['enum', 'file'].indexOf(this.type) != -1) {
                ...
                if ($j(obj.el).prop('readonly') || $j(obj.el).prop('disabled')) {
                    ...
                    $j(obj.helpers.multi).find('input').prop('readonly', true);                    
                } else {
                    div.removeClass('w2ui-readonly');
                    div.css('pointer-events', 'auto').find('li').css('opacity', '1');
                    $j(obj.helpers.multi).find('input').prop('readonly', false);
                }
                // celan
                ...
                if (html != '') {
                    ...
                    var style =
                        'padding-top: ' + $j(this.el).css('padding-top') + ';'+
                        'padding-left: ' + $j(this.el).css('padding-left') + '; ' +
                        'box-sizing: ' + $j(this.el).css('box-sizing') + '; ' +
                        'line-height: ' + $j(this.el).css('line-height') + '; ' +
                        'font-size: ' + $j(this.el).css('font-size') + '; ' +
                        'font-family: ' + $j(this.el).css('font-family') + '; ';
                ...
                div.find('li')
                    .data('mouse', 'out')
                    .on('click', function (event) {
                        var item = selected[$j(event.target).attr('index')];
                        if ($j(event.target).hasClass('nomouse')) return;
                        event.stopPropagation();
                        // trigger event
                        var eventData = obj.trigger({ phase: 'before', type: 'click', target: obj.el, originalEvent: event.originalEvent, item: item });
                        if (eventData.isCancelled === true) return;
                        // default behavior
                        if ($j(event.target).hasClass('w2ui-list-remove')) {
                            if ($j(obj.el).attr('readonly') || $j(obj.el).attr('disabled')) return;
                            // trigger event
                            var eventData = obj.trigger({ phase: 'before', type: 'remove', target: obj.el, originalEvent: event.originalEvent, item: item });
                            if (eventData.isCancelled === true) return;
                            // default behavior
                            $j().w2overlay();
                            selected.splice($j(event.target).attr('index'), 1);
                            $j(obj.el).trigger('change');
                            $j(event.target).parent().fadeOut('fast');
                            setTimeout(function () {
                                obj.refresh();
                                // event after
                                obj.trigger($j.extend(eventData, { phase: 'after' }));
                        ...
                        if (obj.type == 'file' && !$j(event.target).hasClass('w2ui-list-remove')) {
                            var preview = '';
                            if ((/image/i).test(item.type)) { // image
                                preview = '<div style="padding: 3px;">'+
                                    '    <img src="'+ (item.content ? 'data:'+ item.type +';base64,'+ item.content : '') +'" style="max-width: 300px;" '+
                                    '        onload="var w = $j(this).width(); var h = $j(this).height(); '+
                                    '            if (w < 300 & h < 300) return; '+
                                    '            if (w >= h && w > 300) $j(this).width(300);'+
                                    '            if (w < h && h > 300) $j(this).height(300);"'+
                            ...
                            preview += '<div style="padding: 8px;">'+
                                ...
                                '    </table>'+
                                '</div>';
                                $j(event.target).w2overlay(preview);
                        }
                        // event after
                        obj.trigger($j.extend(eventData, { phase: 'after' }));
                    })
                    .on('mouseover', function (event) {
                        ...
                        if ($j(tmp).hasClass('nomouse')) return;
                        if ($j(tmp).data('mouse') == 'out') {
                            var item = selected[$j(tmp).attr('index')];
                            // trigger event
                        ...
                        $j(tmp).data('mouse', 'over');
                    })
                    .on('mouseout', function (event) {
                        var tmp = event.target;
                        if (tmp.tagName != 'LI') tmp = tmp.parentNode;
                        if ($j(tmp).hasClass('nomouse')) return;
                        $j(tmp).data('mouse', 'leaving');
                        setTimeout(function () {
                            if ($j(tmp).data('mouse') == 'leaving') {
                                $j(tmp).data('mouse', 'out');
                                var item = selected[$j(tmp).attr('index')];
                                // trigger event
                ...
                // adjust height
                $j(this.el).height('auto');
                var cntHeight = $j(div).find('> div').height() + w2utils.getSize(div, '+height') * 2;
                if (cntHeight < 26) cntHeight = 26;
                if (cntHeight > options.maxHeight) cntHeight = options.maxHeight;
                if (div.length > 0) div[0].scrollTop = 1000;
                var inpHeight = w2utils.getSize($j(this.el), 'height') - 2;
                if (inpHeight > cntHeight) cntHeight = inpHeight
                $j(div).css({ 'height': cntHeight + 'px', overflow: (cntHeight == options.maxHeight ? 'auto' : 'hidden') });
                if (cntHeight < options.maxHeight) $j(div).prop('scrollTop', 0);
                $j(this.el).css({ 'height' : (cntHeight + 2) + 'px' });
            }
            return (new Date()).getTime() - time;
        },
        ...
        clean: function (val) {
            ...
            if (['int', 'float', 'money', 'currency', 'percent'].indexOf(this.type) != -1) {
                ...
                if (parseFloat(val) == val) {
                    if (options.min !== null && val < options.min) { val = options.min; $j(this.el).val(options.min); }
                    if (options.max !== null && val > options.max) { val = options.max; $j(this.el).val(options.max); }
                }
        ...
        change: function (event) {
            ...
            if (['int', 'float', 'money', 'currency', 'percent'].indexOf(this.type) != -1) {
                // check max/min
                var val     =  $j(this.el).val();
                var new_val = this.format(this.clean($j(this.el).val()));
                // if was modified
                if (val != '' && val != new_val) {
                    $j(this.el).val(new_val).change();
                    // cancel event
                    event.stopPropagation();
                    event.preventDefault();
                    return false;
            ...
            // color
            if (this.type == 'color') {
                var color = '#' + $j(this.el).val();
                if ($j(this.el).val().length != 6 && $j(this.el).val().length != 3) color = '';
                $j(this.el).next().find('div').css('background-color', color);
                if ($j(obj.el).is(':focus')) this.updateOverlay();
            }
            // list, enum
            ...
            if (['date', 'time'].indexOf(this.type) != -1) {
                ...
                if (w2utils.isInt(tmp) && tmp > 1000) {
                    if (this.type == 'time') $j(obj.el).val(w2utils.formatTime(new Date(tmp), options.format)).change();
                    if (this.type == 'date') $j(obj.el).val(w2utils.formatDate(new Date(tmp), options.format)).change();
                }
            }
        },
        ...
        click: function (event) {
            ...
            if (['list', 'combo', 'enum'].indexOf(this.type) != -1) {
                if (!$j(this.el).is(':focus')) this.focus(event);
            }
        ...
        focus: function (event) {
            ...
            if (['color', 'date', 'time'].indexOf(obj.type) !== -1) {
                if ($j(obj.el).attr('readonly') || $j(obj.el).attr('disabled')) return;
                if ($j("#w2ui-overlay").length > 0) $j('#w2ui-overlay')[0].hide();
            ...
            if (['list', 'combo', 'enum'].indexOf(obj.type) != -1) {
                ...
                if ($j("#w2ui-overlay").length > 0) $j('#w2ui-overlay')[0].hide();
                setTimeout(function () {
                    if (obj.type == 'list' && $j(obj.el).is(':focus')) {
                        $j(obj.helpers.focus).find('input').focus();
                        return;
                    }
                    obj.search();
            ...
            // file
            if (obj.type == 'file') {
                $j(obj.helpers.multi).css({ 'outline': 'auto 5px #7DB4F3', 'outline-offset': '-2px' });
            }
        },
        ...
        blur: function (event) {
            ...
            if (['int', 'float', 'money', 'currency', 'percent'].indexOf(obj.type) != -1) {
                if (val !== '' && !obj.checkType(val)) {
                    ...
                    if (options.silent === false) {
                        $j(obj.el).w2tag('Not a valid number');
                        setTimeout(function () { $j(obj.el).w2tag(''); }, 3000);
                    }
                }
            }
            // date or time
            if (['date', 'time'].indexOf(obj.type) != -1) {
                ...
                if (val !== '' && !obj.inRange(obj.el.value)) {
                    $j(obj.el).val('').removeData('selected').change();
                    if (options.silent === false) {
                        $j(obj.el).w2tag('Not in range');
                        setTimeout(function () { $j(obj.el).w2tag(''); }, 3000);
                    }
                } else {
                    if (obj.type == 'date' && val !== '' && !w2utils.isDate(obj.el.value, options.format)) {
                        $j(obj.el).val('').removeData('selected').change();
                        if (options.silent === false) {
                            $j(obj.el).w2tag('Not a valid date');
                            setTimeout(function () { $j(obj.el).w2tag(''); }, 3000);
                        }
                    }
                    if (obj.type == 'time' && val !== '' && !w2utils.isTime(obj.el.value)) {
                        $j(obj.el).val('').removeData('selected').change();
                        if (options.silent === false) {
                            $j(obj.el).w2tag('Not a valid time');
                            setTimeout(function () { $j(obj.el).w2tag(''); }, 3000);
                        }
                    }
                }
            ...
            // clear search input
            if (obj.type == 'enum') {
                $j(obj.helpers.multi).find('input').val('').width(20);
            }
            // file
            if (obj.type == 'file') {
                $j(obj.helpers.multi).css({ 'outline': 'none' });
            }
        },
        ...
        keyDown: function (event, extra) {
            ...
            // numeric
            if (['int', 'float', 'money', 'currency', 'percent'].indexOf(obj.type) != -1) {
                if (!options.keyboard || $j(obj.el).attr('readonly')) return;
                var cancel = false;
                var val = parseFloat($j(obj.el).val().replace(options.moneyRE, '')) || 0;
                var inc = options.step;
                if (event.ctrlKey || event.metaKey) inc = 10;
                switch (key) {
                    case 38: // up
                        if (event.shiftKey) break; // no action if shift key is pressed
                        $j(obj.el).val((val + inc <= options.max || options.max === null ? Number((val + inc).toFixed(12)) : options.max)).change();
                        cancel = true;
                        break;
                    case 40: // down
                        if (event.shiftKey) break; // no action if shift key is pressed
                        $j(obj.el).val((val - inc >= options.min || options.min === null ? Number((val - inc).toFixed(12)) : options.min)).change();
                        cancel = true;
                        break;
                }
                if (cancel) {
            ...
            if (obj.type == 'date') {
                ...
                var inc        = 1;
                if (event.ctrlKey || event.metaKey) inc = 10;
                var dt = w2utils.isDate($j(obj.el).val(), options.format, true);
            ...
            if (obj.type == 'time') {
                ...
                if (cancel) {
                    $j(obj.el).val(obj.fromMin(time)).change();
                    event.preventDefault();
                    setTimeout(function () {
                        // set cursor to the end
                        obj.el.setSelectionRange(obj.el.value.length, obj.el.value.length);
            ...
            // color
            if (obj.type == 'color') {
                if ($j(obj.el).attr('readonly')) return;
                // paste
                if (event.keyCode == 86 && (event.ctrlKey || event.metaKey)) {
                    $j(obj.el).prop('maxlength', 7);
                    setTimeout(function () {
                        var val = $j(obj).val();
                        if (val.substr(0, 1) == '#') val = val.substr(1);
                        if (!w2utils.isHex(val)) val = '';
                        $j(obj).val(val).prop('maxlength', 6).change();
                    }, 20);
                }
                if ((event.ctrlKey || event.metaKey) && !event.shiftKey) {
                    ...
                    if ([37, 38, 39, 40].indexOf(key) != -1) {
                        $j(obj.el).val(this.pallete[obj.tmp.cind1][obj.tmp.cind2]).change();
                        event.preventDefault();
                    }
                }
            }
            ...
            if (['list', 'combo', 'enum'].indexOf(obj.type) != -1) {
                if ($j(obj.el).attr('readonly')) return;
                var cancel    = false;
                var selected  = $j(obj.el).data('selected');
                var focus     = $j(obj.helpers.focus).find('input');
                if (obj.type == 'list') {
                ...
                switch (key) {
                    ...
                    case 39: // right
                        ...
                        break;
                    case 13: // enter
                        if ($j('#w2ui-overlay').length == 0) break; // no action if overlay not open
                        var item  = options.items[options.index];
                        var multi = $j(obj.helpers.multi).find('input');
                        if (obj.type == 'enum') {
                            if (item != null) {
                                // trigger event
                            ...
                            if (item) $j(obj.el).data('selected', item).val(item.text).change();
                            if ($j(obj.el).val() == '' && $j(obj.el).data('selected')) $j(obj.el).removeData('selected').val('').change();
                            if (obj.type == 'list') {
                                focus.val('');
                                obj.refresh();
                            }
                    ...
                    case 46: // delete
                        if (obj.type == 'enum' && key == 8) {
                            if ($j(obj.helpers.multi).find('input').val() == '' && selected.length > 0) {
                        ...
                        if (obj.type == 'list' && $j(focus).val() == '') {
                            $j(obj.el).data('selected', {}).change();
                            obj.refresh();
                        }
                        break;
                    case 38: // up
                    ...
                    case 40: // down
                        ...
                        if ($j(input).val() == '' && $j('#w2ui-overlay').length == 0) {
        ...
        keyUp: function (event) {
            if (this.type == 'color') {
                if (event.keyCode == 86 && (event.ctrlKey || event.metaKey)) $j(this).prop('maxlength', 6);
            }
        },
        ...
        request: function (interval) {
            var obj      = this;
            var options  = this.options;
            var search   = $j(obj.el).val() || '';
            // if no url - do nothing
            if (!options.url) return;
            // --
            if (obj.type == 'enum') {
                var tmp = $j(obj.helpers.multi).find('input');
                if (tmp.length == 0) search = ''; else search = tmp.val();
            }
            if (obj.type == 'list') {
                var tmp = $j(obj.helpers.focus).find('input');
                if (tmp.length == 0) search = ''; else search = tmp.val();
            }
            ...
            if (options.url && $j(obj.el).prop('readonly') != true && (
                ...
                obj.tmp.timeout = setTimeout(function () {
                    ...
                    $j.extend(postData, options.postData);
                    ...
                    obj.tmp.xhr = $j.ajax(ajaxOptions)
                        .done(function (data, status, xhr) {
                            // trigger event
        ...
        search: function () {
            ...
            var selected= $j(obj.el).data('selected');
            if (obj.type == 'enum') {
                target = $j(obj.helpers.multi).find('input');
                search = target.val();
                for (var s in selected) { if (selected[s]) ids.push(selected[s].id); }
            }
            if (obj.type == 'list') {
                target = $j(obj.helpers.focus).find('input');
                search = target.val();
                for (var s in selected) { if (selected[s]) ids.push(selected[s].id); }
            }
            // trigger event
            ...
            if (obj.tmp.xhr_loading !== true) {
                ...
                obj.updateOverlay();
                setTimeout(function () { 
                    var html = $j('#w2ui-overlay').html() || '';
                    if (options.markSearch && html.indexOf('$j.fn.w2menuHandler') != -1) { // do not highlight when no items
                        $j('#w2ui-overlay').w2marker(search); 
                    }
                }, 1);
            } else {
        ...
        updateOverlay: function () {
            ...
            if (this.type == 'color') {
                if ($j(obj.el).attr('readonly')) return;
                if ($j('#w2ui-overlay').length == 0) {
                    $j(obj.el).w2overlay(obj.getColorHTML());
                } else {
                    $j('#w2ui-overlay').html(obj.getColorHTML());
                }
                // bind events
                $j('#w2ui-overlay .color')
                    .on('mousedown', function (event) {
                        var color = $j(event.originalEvent.target).attr('name');
                        var index = $j(event.originalEvent.target).attr('index').split(':');
                        obj.tmp.cind1 = index[0];
                        obj.tmp.cind2 = index[1];
                        $j(obj.el).val(color).change();
                        $j(this).html('&#149;');
                    })
                    .on('mouseup', function () {
                        setTimeout(function () {
                            if ($j("#w2ui-overlay").length > 0) $j('#w2ui-overlay').removeData('keepOpen')[0].hide();
                        }, 10);
                    });
            }
            // date
            if (this.type == 'date') {
                if ($j(obj.el).attr('readonly')) return;
                if ($j('#w2ui-overlay').length == 0) {
                    $j(obj.el).w2overlay('<div class="w2ui-reset w2ui-calendar" onclick="event.stopPropagation();"></div>', {
                        css: { "background-color": "#f5f5f5" }
                    });
                }
                var month, year;
                var dt = w2utils.isDate($j(obj.el).val(), obj.options.format, true);
                if (dt) { month = dt.getMonth() + 1; year = dt.getFullYear(); }
                (function refreshCalendar(month, year) {
                    $j('#w2ui-overlay > div > div').html(obj.getMonthHTML(month, year));
                    $j('#w2ui-overlay .w2ui-calendar-title')
                        .on('mousedown', function () {
                            if ($j(this).next().hasClass('w2ui-calendar-jump')) {
                                $j(this).next().remove();
                            } else {
                                var selYear, selMonth;
                                $j(this).after('<div class="w2ui-calendar-jump" style=""></div>');
                                $j(this).next().hide().html(obj.getYearHTML()).fadeIn(200);
                                setTimeout(function () {
                                    $j('#w2ui-overlay .w2ui-calendar-jump')
                                        .find('.w2ui-jump-month, .w2ui-jump-year')
                                        .on('click', function () {
                                            if ($j(this).hasClass('w2ui-jump-month')) {
                                                $j(this).parent().find('.w2ui-jump-month').removeClass('selected');
                                                $j(this).addClass('selected');
                                                selMonth = $j(this).attr('name');
                                            }
                                            if ($j(this).hasClass('w2ui-jump-year')) {
                                                $j(this).parent().find('.w2ui-jump-year').removeClass('selected');
                                                $j(this).addClass('selected');
                                                selYear = $j(this).attr('name');
                                            }
                                            if (selYear != null && selMonth != null) {
                                                $j('#w2ui-overlay .w2ui-calendar-jump').fadeOut(100);
                                                setTimeout(function () { refreshCalendar(parseInt(selMonth)+1, selYear); }, 100);
                                            }
                                        });
                                    $j('#w2ui-overlay .w2ui-calendar-jump >:last-child').prop('scrollTop', 2000);
                                }, 1);
                            }
                        });
                    $j('#w2ui-overlay .w2ui-date')
                        .on('mousedown', function () {
                            var day = $j(this).attr('date');
                            $j(obj.el).val(day).change();
                            $j(this).css({ 'background-color': '#B6D5FB', 'border-color': '#aaa' });
                        })
                        .on('mouseup', function () {
                            setTimeout(function () {
                                if ($j("#w2ui-overlay").length > 0) $j('#w2ui-overlay').removeData('keepOpen')[0].hide();
                    ...
                    $j('#w2ui-overlay .previous').on('mousedown', function () {
                        var tmp = obj.options.current.split('/');
                        tmp[0]  = parseInt(tmp[0]) - 1;
                        refreshCalendar(tmp[0], tmp[1]);
                    });
                    $j('#w2ui-overlay .next').on('mousedown', function () {
                        var tmp = obj.options.current.split('/');
                        tmp[0]  = parseInt(tmp[0]) + 1;
                        refreshCalendar(tmp[0], tmp[1]);
                    });
            ...
            if (this.type == 'time') {
                if ($j(obj.el).attr('readonly')) return;
                if ($j('#w2ui-overlay').length == 0) {
                    $j(obj.el).w2overlay('<div class="w2ui-reset w2ui-calendar-time" onclick="event.stopPropagation();"></div>', {
                        css: { "background-color": "#fff" }
                    });
                }
                var h24 = (this.options.format == 'h24' ? true : false);
                $j('#w2ui-overlay > div').html(obj.getHourHTML());
                $j('#w2ui-overlay .w2ui-time')
                    .on('mousedown', function (event) {
                        $j(this).css({ 'background-color': '#B6D5FB', 'border-color': '#aaa' });
                        var hour = $j(this).attr('hour');
                        $j(obj.el).val((hour > 12 && !h24 ? hour - 12 : hour) + ':00' + (!h24 ? (hour < 12 ? ' am' : ' pm') : '')).change();
                    })
                    .on('mouseup', function () {
                        var hour = $j(this).attr('hour');
                        if ($j("#w2ui-overlay").length > 0) $j('#w2ui-overlay')[0].hide();
                        $j(obj.el).w2overlay('<div class="w2ui-reset w2ui-calendar-time"></div>', { css: { "background-color": "#fff" } });
                        $j('#w2ui-overlay > div').html(obj.getMinHTML(hour));
                        $j('#w2ui-overlay .w2ui-time')
                            .on('mousedown', function () {
                                $j(this).css({ 'background-color': '#B6D5FB', 'border-color': '#aaa' });
                                var min = $j(this).attr('min');
                                $j(obj.el).val((hour > 12 && !h24 ? hour - 12 : hour) + ':' + (min < 10 ? 0 : '') + min + (!h24 ? (hour < 12 ? ' am' : ' pm') : '')).change();
                            })
                            .on('mouseup', function () {
                                setTimeout(function () { if ($j("#w2ui-overlay").length > 0) $j('#w2ui-overlay').removeData('keepOpen')[0].hide(); }, 10);
                            });
                    });
            }
            // list
            if (['list', 'combo', 'enum'].indexOf(this.type) != -1) {
                ...
                var input = this.el;
                if (this.type == 'enum') {
                    el    = $j(this.helpers.multi);
                    input = $j(el).find('input');
                }
                if (this.type == 'list') {
                    input = $j(this.helpers.focus).find('input');
                }
                if ($j(input).is(':focus')) {
                    if (options.openOnFocus === false && $j(input).val() == '' && obj.tmp.force_open !== true) {
                        $j().w2overlay();
                        return;
                    }
                    ...
                    if ($j(input).val() != '') delete obj.tmp.force_open;
                    if ($j('#w2ui-overlay').length == 0) options.index = 0;
                    var msgNoItems = w2utils.lang('No matches');
                    if (options.url != null && $j(input).val().length < options.minLength && obj.tmp.emptySet !== true) msgNoItems = options.minLength + ' ' + w2utils.lang('letters or more...');
                    if (options.url != null && $j(input).val() == '' && obj.tmp.emptySet !== true) msgNoItems = w2utils.lang('Type to search....');
                    $j(el).w2menu('refresh', $j.extend(true, {}, options, {
                        search     : false,
                        ...
                        onSelect: function (event) {
                            if (obj.type == 'enum') {
                                ...
                                if (event.item) {
                                    ...
                                    $j(obj.el).data('selected', selected).change();
                                    $j(obj.helpers.multi).find('input').val('').width(20);
                                    obj.refresh();
                                    if ($j("#w2ui-overlay").length > 0) $j('#w2ui-overlay')[0].hide();
                                    // event after
                                ...
                                $j(obj.el).data('selected', event.item).val(event.item.text).change();
            ...
            if (this.type == 'date') {
                ...
                if (dt) {
                    ...
                    if (this.options.start || this.options.end) {
                        var st = (typeof this.options.start == 'string' ? this.options.start : $j(this.options.start).val());
                        var en = (typeof this.options.end == 'string' ? this.options.end : $j(this.options.end).val());
                    ...
                    if (this.options.blocked && $j.inArray(str, this.options.blocked) != -1) inRange = false;
                }
            }
            if (this.type == 'time') {
                if (this.options.start || this.options.end) {
        ...
        */
        ...
        addPrefix: function () {
            ...
            setTimeout(function () {
                ...
                var helper;
                var tmp = $j(obj.el).data('tmp') || {};
                if (tmp['old-padding-left']) $j(obj.el).css('padding-left', tmp['old-padding-left']);
                tmp['old-padding-left'] = $j(obj.el).css('padding-left');
                $j(obj.el).data('tmp', tmp);
                // remove if already displaed
                if (obj.helpers.prefix) $j(obj.helpers.prefix).remove();
                if (obj.options.prefix !== '') {
                    ...
                    helper = $j(obj.el).prev();
                    helper
                        .css({
                            'color'          : $j(obj.el).css('color'),
                            'font-family'    : $j(obj.el).css('font-family'),
                            'font-size'      : $j(obj.el).css('font-size'),
                            'padding-top'    : $j(obj.el).css('padding-top'),
                            'padding-bottom' : $j(obj.el).css('padding-bottom'),
                            'padding-left'   : $j(obj.el).css('padding-left'),
                            'padding-right'  : 0,
                            'margin-top'     : (parseInt($j(obj.el).css('margin-top'), 10) + 2) + 'px',
                            'margin-bottom'  : (parseInt($j(obj.el).css('margin-bottom'), 10) + 1) + 'px',
                            'margin-left'    : $j(obj.el).css('margin-left'),
                            'margin-right'   : 0
                        })
                        .on('click', function (event) {
                            if (obj.options.icon && typeof obj.onIconClick == 'function') {
                                // event before
                                var eventData = obj.trigger({ phase: 'before', type: 'iconClick', target: obj.el, el: $j(this).find('span.w2ui-icon')[0] });
                                if (eventData.isCancelled === true) return;
                    ...
                    $j(obj.el).css('padding-left', (helper.width() + parseInt($j(obj.el).css('padding-left'), 10)) + 'px');
                    // remember helper
        ...
        addSuffix: function () {
            ...
            setTimeout(function () {
                if (obj.type === 'clear') return;
                var tmp = $j(obj.el).data('tmp') || {};
                if (tmp['old-padding-right']) $j(obj.el).css('padding-right', tmp['old-padding-right']);
                tmp['old-padding-right'] = $j(obj.el).css('padding-right');
                $j(obj.el).data('tmp', tmp);
                pr = parseInt($j(obj.el).css('padding-right'), 10);
                if (obj.options.arrows) {
                    // remove if already displaed
                    if (obj.helpers.arrows) $j(obj.helpers.arrows).remove();
                    // add fresh
                    $j(obj.el).after(
                    ...
                    helper = $j(obj.el).next();
                    helper.css({
                            'color'         : $j(obj.el).css('color'),
                            'font-family'   : $j(obj.el).css('font-family'),
                            'font-size'     : $j(obj.el).css('font-size'),
                            'height'        : ($j(obj.el).height() + parseInt($j(obj.el).css('padding-top'), 10) + parseInt($j(obj.el).css('padding-bottom'), 10) ) + 'px',
                            'padding'       : 0,
                            'margin-top'    : (parseInt($j(obj.el).css('margin-top'), 10) + 1) + 'px',
                            'margin-bottom' : 0,
                            'border-left'   : '1px solid silver'
                        })
                        .css('margin-left', '-'+ (helper.width() + parseInt($j(obj.el).css('margin-right'), 10) + 12) + 'px')
                        .on('mousedown', function (event) {
                            $j('body').on('mouseup', tmp);
                            $j('body').data('_field_update_timer', setTimeout(update, 700));
                            update(false);
                            // timer function
                            function tmp() {
                                clearTimeout($j('body').data('_field_update_timer'));
                                $j('body').off('mouseup', tmp);
                            }
                            // update function
                            function update(notimer) {
                                $j(obj.el).focus();
                                obj.keyDown($j.Event("keydown"), {
                                    keyCode : ($j(event.target).attr('type') == 'up' ? 38 : 40)
                                });
                                if (notimer !== false) $j('body').data('_field_update_timer', setTimeout(update, 60));
                            }
                        });
                    pr += helper.width() + 12;
                    $j(obj.el).css('padding-right', pr + 'px');
                    // remember helper
                    obj.helpers.arrows = helper;
                }
                if (obj.options.suffix !== '') {
                    // remove if already displaed
                    if (obj.helpers.suffix) $j(obj.helpers.suffix).remove();
                    // add fresh
                    $j(obj.el).after(
                        '<div class="w2ui-field-helper">'+
                            obj.options.suffix +
                    ...
                    helper
                        .css({
                            ...
                            'padding-bottom' : $j(obj.el).css('padding-bottom'),
                            'padding-left'   : '3px',
                            'padding-right'  : $j(obj.el).css('padding-right'),
                            'margin-top'     : (parseInt($j(obj.el).css('margin-top'), 10) + 2) + 'px',
                            'margin-bottom'  : (parseInt($j(obj.el).css('margin-bottom'), 10) + 1) + 'px'
                        })
                        .on('click', function (event) {
                    ...
                    helper.css('margin-left', '-'+ (w2utils.getSize(helper, 'width') + parseInt($j(obj.el).css('margin-right'), 10) + 2) + 'px');
                    pr += helper.width() + 3;
                    $j(obj.el).css('padding-right', pr + 'px');
                    // remember helper
        ...
        addFocus: function () {
            ...
            var html =
                ...
                '<div>';
            $j(obj.el).attr('tabindex', -1).before(html);
            var helper = $j(obj.el).prev();
            obj.helpers.focus = helper;
            helper.css({
                    width           : $j(obj.el).width(),
                    "margin-top"    : $j(obj.el).css('margin-top'),
                    "margin-left"   : (parseInt($j(obj.el).css('margin-left')) + parseInt($j(obj.el).css('padding-left'))) + 'px',
                    "margin-bottom" : $j(obj.el).css('margin-bottom'),
                    "margin-right"  : $j(obj.el).css('margin-right')
                })
                .find('input')
                .css({
                    cursor   : 'default',
                    ...
                    padding  : $j(obj.el).css('padding-top'),
                    "padding-left"     : 0,
            ...
            helper.find('input')
                .on('click', function (event) {
                    if ($j('#w2ui-overlay').length == 0) obj.focus(event);
                    event.stopPropagation();
                })
                .on('focus', function (event) {
                    $j(obj.el).css({ 'outline': 'auto 5px #7DB4F3', 'outline-offset': '-2px' });
                    $j(this).val('');
                    $j(obj.el).triggerHandler('focus');
                    if (event.stopPropagation) event.stopPropagation(); else event.cancelBubble = true;
                })
                .on('blur', function (event) {
                    $j(obj.el).css('outline', 'none');
                    $j(this).val('');
                    obj.refresh(); 
                    $j(obj.el).triggerHandler('blur');
                    if (event.stopPropagation) event.stopPropagation(); else event.cancelBubble = true;
                })
            ...
            // MAIN div
            helper.on('click', function (event) { $j(this).find('input').focus(); });
            obj.refresh();
        },    
        ...
        addMulti: function () {
            ...
            var margin =
                ...
                'width          : ' + (w2utils.getSize(obj.el, 'width')
                                    - parseInt($j(obj.el).css('margin-left'), 10)
                                    - parseInt($j(obj.el).css('margin-right'), 10))
                                    + 'px;';
            if (obj.type == 'enum') {
                html =  '<div class="w2ui-field-helper w2ui-list" style="'+ margin + '; box-sizing: border-box">'+
                        ...
                        '            <input type="text" style="width: 20px" autocomplete="off" '+ ($j(obj.el).attr('readonly') ? 'readonly': '') + '>'+
                        '        </li>'
                        '    </ul>'+
                        '    </div>'+
                        '</div>';
            ...
            var div    = $j(obj.el).prev();
            obj.helpers.multi = div;
            if (obj.type == 'enum') {
                $j(obj.el).attr('tabindex', -1);
                // INPUT events
                div.find('input')
                    .on('click', function (event) {
                        if ($j('#w2ui-overlay').length == 0) obj.focus(event);
                        $j(obj.el).triggerHandler('click');
                    })
                    .on('focus', function (event) {
                        $j(div).css({ 'outline': 'auto 5px #7DB4F3', 'outline-offset': '-2px' });
                        $j(obj.el).triggerHandler('focus');
                        if (event.stopPropagation) event.stopPropagation(); else event.cancelBubble = true;
                    })
                    .on('blur', function (event) {
                        $j(div).css('outline', 'none');
                        $j(obj.el).triggerHandler('blur');
                        if (event.stopPropagation) event.stopPropagation(); else event.cancelBubble = true;
                    })
                    .on('keyup',    function (event) { obj.keyUp(event) })
                ...
                // MAIN div
                div.on('click', function (event) { $j(this).find('input').focus(); });
            }
            if (obj.type == 'file') {
                $j(obj.el).css('outline', 'none');
                div.on('click', function (event) {
                    ...
                    .on('dragenter', function (event) {
                        if ($j(obj.el).attr('readonly')) return;
                        $j(div).addClass('w2ui-file-dragover');
                    })
                    .on('dragleave', function (event) {
                        if ($j(obj.el).attr('readonly')) return;
                        var tmp = $j(event.target).parents('.w2ui-field-helper');
                        if (tmp.length == 0) $j(div).removeClass('w2ui-file-dragover');
                    })
                    .on('drop', function (event) {
                        if ($j(obj.el).attr('readonly')) return;
                        $j(div).removeClass('w2ui-file-dragover');
        ...
        addFile: function (file) {
            ...
            if (options.maxFileSize !== 0 && newItem.size > options.maxFileSize) {
                ...
                if (options.silent === false) $j(obj.el).w2tag(err);
            ...
            for (var i = 0; i < 8; i++) {
                ...
                for (var j = 0; j < 8; j++) {
                    html += '<td>'+
                            ...
                            '        '+ ($j(this.el).val() == this.pallete[i][j] ? '&#149;' : '&nbsp;')+
                            '    </div>'+
                            '</td>';
                }
                html += '</tr>';
    ...
    $j.extend(w2field.prototype, w2utils.event);
    w2obj.field = w2field;
...
(function () {
    var w2form = function(options) {
        ...
        $j.extend(true, this, w2obj.form, options);
    };

    // ====================================================
    // -- Registers as a jQuery plugin

    $j.fn.w2form = function(method) {
        if (typeof method === 'object' || !method ) {
            var obj = this;
            // check name parameter
            ...
            $j.extend(object, { record: {}, original: {}, fields: [], tabs: {}, toolbar: {}, handlers: [] });
            if ($j.isArray(tabs)) {
                $j.extend(true, object.tabs, { tabs: [] });
                for (var t in tabs) {
                    var tmp = tabs[t];
            ...
            // reassign variables
            for (var p in fields) {
                var field = $j.extend(true, {}, fields[p]);
            ...
            for (var p in record) {
                if ($j.isPlainObject(record[p])) {
                    object.record[p] = $j.extend(true, {}, record[p]);
                } else {
                    object.record[p] = record[p];
                }
            }
            for (var p in original) {
                if ($j.isPlainObject(original[p])) {
                    object.original[p] = $j.extend(true, {}, original[p]);
                } else {
            ...
            // render if necessary
            if (object.formURL != '') {
                $j.get(object.formURL, function (data) { // should always be $j.get as it is template
                    object.formHTML = data;
                    object.isGenerated = true;
                    if ($j(object.box).length != 0 || data.length != 0) {
                        $j(object.box).html(data);
                        object.render(object.box);
                    }
                });
            } else if (object.formHTML != '') {
                // it is already loaded into formHTML
            } else if ($j(this).length != 0 && $j.trim($j(this).html()) != '') {
                object.formHTML = $j(this).html();
            ...
            if (object.formURL == '') {
                ...
                $j(object.box).html(object.formHTML);
                object.isGenerated = true;
                object.render(object.box);
            }
            return object;
    ...
    w2form.prototype = {
        ...
        validate: function (showErrors) {
            ...
            $j().w2tag(); // hide all tags before validating
            ...
            for (var f in this.fields) {
                ...
                if (field.required && (val === '' || ($j.isArray(val) && val.length == 0) || ($j.isPlainObject(val) && $j.isEmptyObject(val)))) {
            ...
            if (showErrors) for (var e in eventData.errors) {
                ...
                if (err.field.type == 'radio') { // for radio and checkboxes
                    $j($j(err.field.el).parents('div')[0]).w2tag(err.error, { "class": 'w2ui-error' });
                } else if (['enum', 'file'].indexOf(err.field.type) != -1) {
                    (function (err) {
                        setTimeout(function () {
                            var fld = $j(err.field.el).data('w2field').helpers.multi;
                            $j(err.field.el).w2tag(err.error);
                            $j(fld).addClass('w2ui-error');
                        }, 1);
                    })(err);
                } else {
                    $j(err.field.el).w2tag(err.error, { "class": 'w2ui-error' });
                }
            ...
            var differ = function(record, original, result) {
                for (var i in record) {
                    if (typeof record[i] == "object") {
                        ...
                        if (!result[i] || $j.isEmptyObject(result[i])) delete result[i];
        ...
        request: function (postData, callBack) { // if (1) param then it is call back if (2) then postData and callBack
            ...
            if (w2utils.settings.dataType == 'HTTP') {
                ajaxOptions.data = String($j.param(ajaxOptions.data, false)).replace(/%5B/g, '[').replace(/%5D/g, ']');
            }
            if (w2utils.settings.dataType == 'RESTFULL') {
                ajaxOptions.type = 'GET';
                ajaxOptions.data = String($j.param(ajaxOptions.data, false)).replace(/%5B/g, '[').replace(/%5D/g, ']');
                    ...
                    if (status != 'error') {
                        ...
                        if (typeof responseText != 'undefined' && responseText != '') {
                            ...
                            if (data['status'] == 'error') {
                                ...
                                obj.record   = $j.extend({}, data.record);
                                obj.original = $j.extend({}, data.record);
                            }
                        }
                    } else {
        ...
        save: function (postData, callBack) {
            var obj = this;
            $j(this.box).find(':focus').change(); // trigger onchange
            // check for multiple params
            if (typeof postData == 'function') {
                callBack = postData;
                postData = null;
            ...
            setTimeout(function () {
                ...
                // append other params
                $j.extend(params, obj.postData);
                $j.extend(params, postData);
                params.record = $j.extend(true, {}, obj.record);
                // event before
                    ...
                    xhr : function() {
                        ...
                        xhr.upload.addEventListener("progress", function(evt) {
                            if (evt.lengthComputable) {
                                ...
                                $j('#'+ obj.name + '_progress').text(''+ percent + '%');
                            }
                        }, false);
                        return xhr;
                    }
                ...
                obj.last.xhr = $j.ajax(ajaxOptions)
                    .done(function (data, status, xhr) {
                        obj.unlock();
                        // event before
                        ...
                        if (status != 'error') {
                            ...
                            if (typeof responseText != 'undefined' && responseText != '') {
                                ...
                                if (data['status'] == 'error') {
                                    ...
                                    obj.original = $j.extend({}, obj.record);
                                }
                            }
                        } else {
                            obj.error('AJAX Error ' + xhr.status + ': '+ xhr.statusText);
        ...
        goto: function (page) {
            ...
            if ($j(this.box).data('auto-size') === true) $j(this.box).height(0);
            this.refresh();
        },
        ...
        generateHTML: function () {
            ...
            for (var f in this.fields) {
                ...
                field.html = $j.extend(true, { caption: '', span: 6, attr: '', text: '', page: 0 }, field.html);
            ...
            // buttons if any
            var buttons = '';
            if (!$j.isEmptyObject(this.actions)) {
                var addClass = '';
        ...
        resize: function () {
            ...
            // default behaviour
            var main    = $j(this.box).find('> div');
            var header  = $j(this.box).find('> div .w2ui-form-header');
            var toolbar = $j(this.box).find('> div .w2ui-form-toolbar');
            var tabs    = $j(this.box).find('> div .w2ui-form-tabs');
            var page    = $j(this.box).find('> div .w2ui-page');
            var cpage   = $j(this.box).find('> div .w2ui-page.page-'+ this.page);
            var dpage   = $j(this.box).find('> div .w2ui-page.page-'+ this.page + ' > div');
            var buttons = $j(this.box).find('> div .w2ui-buttons');
            // if no height, calculate it
            resizeElements();
            if (parseInt($j(this.box).height()) == 0 || $j(this.box).data('auto-size') === true) {
                $j(this.box).height(
                    (header.length > 0 ? w2utils.getSize(header, 'height') : 0) +
                    ((typeof this.tabs === 'object' && $j.isArray(this.tabs.tabs) && this.tabs.tabs.length > 0) ? w2utils.getSize(tabs, 'height') : 0) +
                    ((typeof this.toolbar == 'object' && $j.isArray(this.toolbar.items) && this.toolbar.items.length > 0) ? w2utils.getSize(toolbar, 'height') : 0) +
                    (page.length > 0 ? w2utils.getSize(dpage, 'height') + w2utils.getSize(cpage, '+height') + 12 : 0) +  // why 12 ???
                    (buttons.length > 0 ? w2utils.getSize(buttons, 'height') : 0)
                );
                $j(this.box).data('auto-size', true);
            }
            resizeElements();
            // event after
            obj.trigger($j.extend(eventData, { phase: 'after' }));
            ...
            function resizeElements() {
                // resize elements
                main.width($j(obj.box).width()).height($j(obj.box).height());
                toolbar.css('top', (obj.header != '' ? w2utils.getSize(header, 'height') : 0));
                tabs.css('top', (obj.header != '' ? w2utils.getSize(header, 'height') : 0)
                              + ((typeof obj.toolbar == 'object' && $j.isArray(obj.toolbar.items) && obj.toolbar.items.length > 0) ? w2utils.getSize(toolbar, 'height') : 0));
                page.css('top', (obj.header != '' ? w2utils.getSize(header, 'height') : 0)
                              + ((typeof obj.toolbar == 'object' && $j.isArray(obj.toolbar.items) && obj.toolbar.items.length > 0) ? w2utils.getSize(toolbar, 'height') + 5 : 0)
                              + ((typeof obj.tabs === 'object' && $j.isArray(obj.tabs.tabs) && obj.tabs.tabs.length > 0) ? w2utils.getSize(tabs, 'height') + 5 : 0));
        ...
        refresh: function () {
            ...
            var obj = this;
            if (!this.box) return;
            if (!this.isGenerated || typeof $j(this.box).html() == 'undefined') return;
            // update what page field belongs
            $j(this.box).find('input, textarea, select').each(function (index, el) {
                var name  = (typeof $j(el).attr('name') != 'undefined' ? $j(el).attr('name') : $j(el).attr('id'));
                var field = obj.get(name);
                if (field) {
                    // find page
                    var div = $j(el).parents('.w2ui-page');
                    if (div.length > 0) {
                        for (var i = 0; i < 100; i++) {
            ...
            if (eventData.isCancelled === true) return;
            // default action
            $j(this.box).find('.w2ui-page').hide();
            $j(this.box).find('.w2ui-page.page-' + this.page).show();
            $j(this.box).find('.w2ui-form-header').html(this.header);
            // refresh tabs if needed
            if (typeof this.tabs === 'object' && $j.isArray(this.tabs.tabs) && this.tabs.tabs.length > 0) {
                $j('#form_'+ this.name +'_tabs').show();
                this.tabs.active = this.tabs.tabs[this.page].id;
                this.tabs.refresh();
            } else {
                $j('#form_'+ this.name +'_tabs').hide();
            }
            // refresh tabs if needed
            if (typeof this.toolbar == 'object' && $j.isArray(this.toolbar.items) && this.toolbar.items.length > 0) {
                $j('#form_'+ this.name +'_toolbar').show();
                this.toolbar.refresh();
            } else {
                $j('#form_'+ this.name +'_toolbar').hide();
            }
            ...
            for (var f in this.fields) {
                ...
                if (typeof field.name == 'undefined' && typeof field.field != 'undefined') field.name = field.field;
                if (typeof field.field == 'undefined' && typeof field.name != 'undefined') field.field = field.name;
                field.$el = $j(this.box).find('[name="'+ String(field.name).replace(/\\/g, '\\\\') +'"]');
                field.el  = field.$el[0];
                ...
                var tmp = $j(field).data('w2field');
                if (tmp) tmp.clear();
                $j(field.$el).off('change').on('change', function () {
                    var value_new      = this.value;
                    var value_previous = obj.record[this.name] ? obj.record[this.name] : '';
                    var field          = obj.get(this.name);
                    if (['list', 'enum', 'file'].indexOf(field.type) != -1 && $j(this).data('selected')) {
                        var nv = $j(this).data('selected');
                        var cv = obj.record[this.name];
                        if ($j.isArray(nv)) {
                            value_new = [];
                            for (var i in nv) value_new[i] = $j.extend(true, {}, nv[i]); // clone array
                        }
                        if ($j.isPlainObject(nv)) {
                            value_new = $j.extend(true, {}, nv); // clone object
                        }
                        if ($j.isArray(cv)) {
                            value_previous = [];
                            for (var i in cv) value_previous[i] = $j.extend(true, {}, cv[i]); // clone array
                        }
                        if ($j.isPlainObject(cv)) {
                            value_previous = $j.extend(true, {}, cv); // clone object
                        }
                    }
                    if (field.type == 'toggle') value_new = ($j(this).prop('checked') ? 1 : 0);
                    // clean extra chars
                    if (['int', 'float', 'percent', 'money', 'currency'].indexOf(field.type) != -1) {
                        value_new = $j(this).data('w2field').clean(value_new);
                    }
                    ...
                    if (eventData.isCancelled === true) {
                        $j(this).val(obj.record[this.name]); // return previous value
                        return;
                    }
                    // default action
                    var val = this.value;
                    ...
                    if (['enum', 'file'].indexOf(field.type) != -1) {
                        if (val.length > 0) {
                            var fld = $j(field.el).data('w2field').helpers.multi;
                            $j(fld).removeClass('w2ui-error');
                        }
                    }
                ...
                if (field.required) {
                    $j(field.el).parent().parent().addClass('w2ui-required');
                } else {
                    $j(field.el).parent().parent().removeClass('w2ui-required');
                }
            }
            // attach actions on buttons
            $j(this.box).find('button, input[type=button]').each(function (index, el) {
                $j(el).off('click').on('click', function (event) {
            ...
            for (var f in this.fields) {
                ...
                switch (field.type) {
                    ...
                    case 'time':
                        field.el.value = value;
                        $j(field.el).w2field($j.extend({}, field.options, { type: field.type }));
                        break;
                    case 'toggle':
                        if (w2utils.isFloat(value)) value = parseFloat(value);
                        $j(field.el).prop('checked', (value ? true : false));
                    ...
                    case 'combo':
                        if (field.type == 'list') {
                            var tmp_value = ($j.isPlainObject(value) ? value.id : value);
                            // normalized options
                            var items = field.options.items;
                            if ($j.isArray(items) && items.length > 0 && !$j.isPlainObject(items[0])) {
                            ...
                            for (var i in field.options.items) {
                                ...
                                if (item.id == tmp_value) {
                                    value = $j.extend(true, {}, item);
                                    obj.record[field.name] = value;
                                    break;
                                }
                            }
                        } else if (field.type == 'combo' && !$j.isPlainObject(value)) {
                            field.el.value = value;
                        } else if ($j.isPlainObject(value) && typeof value.text != 'undefined') {
                            field.el.value = value.text;
                        } else {
                            field.el.value = '';
                        }
                        if (!$j.isPlainObject(value)) value = {};
                        $j(field.el).w2field($j.extend({}, field.options, { type: field.type, selected: value }));
                        break;
                    case 'enum':
                    case 'file':
                        if (!$j.isArray(value)) value = [];
                        $j(field.el).w2field($j.extend({}, field.options, { type: field.type, selected: value }));
                        break;
                    ...
                    case 'select':
                        ...
                        if (typeof items != 'undefined' && items.length > 0) {
                            ...
                            $j(field.el).html('');
                            for (var it in items) {
                                $j(field.el).append('<option value="'+ items[it].id +'">' + items[it].text + '</option');
                            }
                        }
                        $j(field.el).val(value);
                        break;
                    case 'radio':
                        $j(field.$el).prop('checked', false).each(function (index, el) {
                            if ($j(el).val() == value) $j(el).prop('checked', true);
                        });
                        break;
                    case 'checkbox':
                        $j(field.el).prop('checked', value ? true : false);
            ...
            // wrap pages in div
            var tmp = $j(this.box).find('.w2ui-page');
            for (var i = 0; i < tmp.length; i++) {
                if ($j(tmp[i]).find('> *').length > 1) $j(tmp[i]).wrapInner('<div></div>');
            }
            // event after
            this.trigger($j.extend(eventData, { phase: 'after' }));
            this.resize();
        ...
        render: function (box) {
            ...
            if (typeof box == 'object') {
                ...
                if ($j(this.box).find('#form_'+ this.name +'_tabs').length > 0) {
                    $j(this.box).removeAttr('name')
            ...
            if (eventData.isCancelled === true) return;
            // default actions
            if ($j.isEmptyObject(this.original) && !$j.isEmptyObject(this.record)) {
                this.original = $j.extend(true, {}, this.record);
            }
            var html =  '<div>' +
                        ...
                        '    <div id="form_'+ this.name +'_tabs" class="w2ui-form-tabs"></div>' +
                            this.formHTML +
                        '</div>';
            $j(this.box).attr('name', this.name)
            ...
            if (typeof this.toolbar.render !== 'function') {
                this.toolbar = $j().w2toolbar($j.extend({}, this.toolbar, { name: this.name +'_toolbar', owner: this }));
                this.toolbar.on('click', function (event) {
                    var eventData = obj.trigger({ phase: 'before', type: 'toolbar', target: event.target, originalEvent: event });
                    if (eventData.isCancelled === true) return;
                    // no default action
            ...
            if (typeof this.toolbar == 'object' && typeof this.toolbar.render == 'function') {
                this.toolbar.render($j('#form_'+ this.name +'_toolbar')[0]);
            }
            // init tabs regardless it is defined or not
            if (typeof this.tabs.render !== 'function') {
                this.tabs = $j().w2tabs($j.extend({}, this.tabs, { name: this.name +'_tabs', owner: this }));
                this.tabs.on('click', function (event) {
                    obj.goto(this.get(event.target, true));
                });
            }
            if (typeof this.tabs == 'object' && typeof this.tabs.render == 'function') {
                this.tabs.render($j('#form_'+ this.name +'_tabs')[0]);
            }
            // event after
            this.trigger($j.extend(eventData, { phase: 'after' }));
            ...
            if ($j('.w2ui-layout').length == 0) { // if there is layout, it will send a resize event
                this.tmp_resize = function (event) { w2ui[obj.name].resize(); }
                $j(window).off('resize', 'body').on('resize', 'body', this.tmp_resize);
            }
            setTimeout(function () { obj.resize(); obj.refresh(); }, 150); // need timer because resize is on timer
            // focus on load
            function focusEl() {
                var inputs = $j(obj.box).find('input, select, textarea');
        ...
        destroy: function () {
            ...
            // event after
            this.trigger($j.extend(eventData, { phase: 'after' }));
            $j(window).off('resize', 'body')
        }
    };

    $j.extend(w2form.prototype, w2utils.event);
    w2obj.form = w2form;
})();
